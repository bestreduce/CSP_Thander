			...
			if(CheckAttribute(NPChar,"Hold_GenQuest") && !CheckAttribute(pchar,"GenQuest.Hold_GenQuest"))
			{
				switch(sti(NPChar.Hold_GenQuest.variant))
				{
					...
					case 1:
						...
						link.l3 = LinkRandPhrase("Не искушайте моё терпение. Вам уже повезло остаться в живых.",
							"У меня насчёт вас другие планы.","Я думаю, что вы мне ещё пригодитесь.");
						link.l3.go = "exit";
					break;
					...
					case 2: // "выкуп"
						...
						link.l2 = RandPhraseSimple(RandPhraseSimple("Я ещё не решил"+ GetSexPhrase("","а") +", что с вами делать.","Узнаете, когда время придёт."),
			...
			else
			{
				...
				else
				{
					...
					if(NPChar.EncType == "war")
					{
						...
						link.l2 = LinkRandPhrase("Не искушайте моё терпение. Вам уже повезло остаться в живых.",
							RandPhraseSimple("У меня насчёт вас другие планы.","Я думаю, что вы мне ещё пригодитесь."),
				...
				else
				{
					...
					{
						link.l4 = "Чёрт с вами! Ступайте к боцману, он получит соответствующие распоряжения, вас высадят в ближайшей бухте. И помните - следующая наша встреча будет последней.";
						link.l4.go = "free_withoutFee_4";
					}
				}
				link.l5 = "Вы проигнорировали приказ лечь в дрейф и оказали сопротивление, за это будете казнены. Боцман уже получил соответствующие распоряжения.";
