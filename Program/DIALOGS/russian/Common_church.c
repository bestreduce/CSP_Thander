...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			{
				Dialog.Text = "Ты... колдун! Как ты посмел, грязный нечестивец, явиться в храм Божий?! Исчадие ада, гнев Господень да падёт на тебя!";
		...
		case "GenQuest_Church_2_Start_5":
			dialog.text = "Ах, "+ GetSexPhrase("сын мой","дочь моя") +", что может простой священник, связанный обетом непротивления злу? Эти негодяи наверняка уже далеко отсюда - пропивают в тавернах  награбленное, или предаются разврату и беспутству в объятиях блудниц! Надеюсь, там им и придёт конец!";
		...
		case "GenQuest_Church_2_Thief_4_1_1":
			dialog.text = ""+ GetSexPhrase("Сын мой, сударь","Дочь моя, сударыня") +"... Господи прости... такие деньги, такое искушение... слаб человек, слаб! Не устоял... Места себе не находил... Непрестанно возносил молитвы, исполнял епитимию, соблюдал строгий пост... умерщвлял плоть и, недостойный, просил небеса об одном лишь - чтобы живы остались те несчастные, которых ввёл во грех, движимый алчностью...";
				link.l1 = LinkRandPhrase("Святой Отец", "Падре", "Отче") + ", успокойтесь. Главное, что они никому ничего не рассказали... и не расскажут. Кстати, они добровольно отдали мне чашу для причастия, которую я вам и прив"+ GetSexPhrase("ёз","езла") +".";
		...
		case "GenQuest_Church_2_Thief_4_1_3":
			...
			else
			{
				dialog.text = ""+ GetSexPhrase("Сын мой","Дочь моя") +"... Со слезами раскаяния и благодарности принимаю от вас чашу, как знак Божьей милости и прощения. Чистота и благородство вашей души потрясли меня. От всего сердца благословляю вас!";
					...
					link.l1.go = "GenQuest_Church_2_Thief_4_1_4";
					link.l1 = "Благословение, конечно, дело нужное, но неплохо бы подкрепить его чем-то более материальным, так как в дороге я пон"+ GetSexPhrase("ёс","есла") +" немалые траты...";
		...
		if(CheckAttribute(PChar, "GenQuest.ChurchQuest_2.Complete.With_All"))
		{
			...
			link.l2 = LinkRandPhrase("Святой отец", "Падре", "Отче") + ", я прин"+ GetSexPhrase("ёс","есла") +" дурные вести... Мне пришлось потратить много времени, сил и средств, чтобы выследить мерзавцев, но, когда я всё-таки добрал"+ GetSexPhrase("ся","ась") +" до них, оказалось, что они успели пропить и промотать всё, кроме вот этой чаши для причастия...";
		...
		if(CheckAttribute(PChar, "GenQuest.ChurchQuest_2.Complete.Short_With_Mon"))
		{
			if(sti(PChar.money) >= sti(PChar.GenQuest.ChurchQuest_2.MoneyCount))
			{
				link.l1 = "Уверен"+ GetSexPhrase("","а") +", что известия вас обрадуют - я примерно наказал"+ GetSexPhrase("","а") +" грабителей и прив"+ GetSexPhrase("ёз","езла") +" приходские деньги.";
				link.l1.go = "GenQuest_Church_2_Finally_Complete_1_2_1";
			}
	...
	//================================================================================================================
		...
		case "GenQuest_Church_1_Complete_2":
			dialog.text = "Да-да, "+ GetSexPhrase("сын мой","дочь моя") +", да-да. Я вознагражу тебя по достоинству. Вот, возьми это, и помни, что Бог наказывает тех неразумных, которые чрезмерно пекутся о благах земных и поклоняются жёлтому дьяволу!";
		...
		case "NightGuard_result":
			...
			{
				dialog.text = ""+ GetSexPhrase("Сын мой","Дочь моя") +", ты ещё не сторожил"+ GetSexPhrase("","а") +" ни одной ночи. Иди, выполняй свою работу, не мешай мне...";
				link.l1 = "Хорошо, святой отец.";
				link.l1.go = "exit";
			}
			if (sti(pchar.questTemp.different.Church_NightGuard.NightQty) > 0 && sti(pchar.questTemp.different.Church_NightGuard.NightQty) < 3)
			{
				dialog.text = ""+ GetSexPhrase("Сын мой","Дочь моя") +", ты не выполнил"+ GetSexPhrase("","а") +" работу до конца, мы договаривались о трёх ночах...";
		...
		case "NightGuard_fight":
			dialog.text = "Помимо этого, я в силах уменьшить твои грехи перед " + NationNameAblative(sti(npchar.nation)) + ", что и сделаю. К тому же, я буду молиться за твоё здоровье, и, полагаю, скоро ты почувствуешь исцеляющую силу молитвы Господу нашему!";
		...
		case "healthAdd_2":
			...
			link.l2 = "То есть - никак? Хм.. обнадёжили, спасибо.";
			link.l2.go = "exit";
		break;
		...
		case "healthAdd_3":
            ...
            {
    			dialog.text = "Я? Пожалуй, это возможно. Но тебе придётся внести подаяние в наш приход.";
			...
			{
                dialog.text = ""+ GetSexPhrase("Сын мой","Дочь моя") +", я уже молюсь о твоём благополучии. Молитвы займут все моё время до конца месяца.";
		...
		case "healthAdd_5":
			...
			dialog.text = "Хорошо. Твоё здоровье немного улучшится. Но тебе нужно избегать стычек и беречь себя, иначе эффект сойдёт на нет. Мне нужно целый месяц провести в молитвах о твоём бренном теле.";
		...
		case "Prison_Cell_Talk_2":
			dialog.text = "Так ты ещё и главарь преступников? И просишь меня пособничать твоим коварным планам? Не бывать этому! Я немедля буду настаивать на твоём приговоре.";
			link.l1 = "Ну и дурак же ты, а ещё рясу нацепил!";
		...
		case "HostessChurch":
			...
			link.l2 = "Я бы предпоч"+ GetSexPhrase("ёл","ла") +" не называть имён. Ведь может же быть анонимное пожертвование!";
		...
		case "HostessChurch_call":
			if (sti(pchar.reputation) < 10))
			{
				...
				{
					dialog.text = "Сам"+ GetSexPhrase("","а") +" грешни"+ GetSexPhrase("к","ца") +", ещё за грешницу и просишь? О своей душе подумал"+ GetSexPhrase("","а") +" бы лучше, ведь все мы перед Господом предстанем!";
			...
			else
			{
				...
				else
				{
					...
					link.l1 = "Очень жаль, что вы отвергаете её. Очень жаль...";
		...
		case "MC_CaracasChirch":
			dialog.text = ""+ GetSexPhrase("Сын мой","Дочь моя") +", Церковь спасает заблудших. Оборотней, ведьм и колдунов наставляет на путь истинный Святая Инквизиция. Или простые горожане, если Господь изберёт их своим орудием. Кто мы такие, чтобы вмешиваться?";
		...
		case "MC_CaracasChirch_1":
			dialog.text = "Не горячитесь, "+ GetSexPhrase("сын мой","дочь моя") +". Если его душа невинна, то она попадёт в рай! Но если я исповедую исчадие ада\nВпрочем, есть способ проверить. Дьявольское отродье не сможет войти в храм Божий. Если вы приведёте его сюда, и не поразит его кара небесная - конечно, я не откажу ему в исповеди.";
