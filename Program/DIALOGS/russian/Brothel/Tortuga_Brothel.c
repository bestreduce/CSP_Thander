...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
	...
	switch (Dialog.CurrentNode)
	{
		case "quests":
            ...
            {
                dialog.text = "Что вам угодно, "+ GetSexPhrase("сэр","мисс") +"?";
            ...
            {
                dialog.text = NPCStringReactionRepeat("Что вам угодно, "+ GetSexPhrase("сэр","мисс") +"?", "Опять вы?", "Снова по этому же вопросу?", "Ну что ещё?", "block", 0, npchar, Dialog.CurrentNode);
            ...
            if (pchar.questTemp.Azzy == "DestrContract")
            {
    			...
    			Link.l1 = "Меня, Маргарита. Но на этот раз я пришёл с конкретным предложением.";
    			Link.l1.go = "Step_17";
            }
            if (pchar.questTemp.Azzy == "2DestrContract")
            {
                dialog.text = "Опять ты?";
    			Link.l1 = "Опять я. Как насчёт вызова Аззи?";
    			Link.l1.go = "Step_20";
            }
		...
		//==>Голландская линейка, квест №3.
 	 	case "Step_H3_1":
			dialog.text = NPCStringReactionRepeat("Сейчас нет, но будет сегодня точно.", ""+ GetSexPhrase("красавчик","красотка") +", ты "+ GetSexPhrase("спрашивал","спрашивала") +" уже меня о Пьере", "Опять о Пьере?", "Послушай, "+ GetSexPhrase("милый мой","милая моя") +", хватит уже рассуждать о Пьере. Осмотрись, развлекись...", "block", 0, npchar, Dialog.CurrentNode);
        ...
        break;
 	 	case "Step_H3_2":
			dialog.text = ""+ GetSexPhrase("Милый мой","Милая моя") +", мне ли не знать?! Не было ещё дня с момента его возвращения, когда он не появлялся у меня. И каждый день берет новую девку, хоть за такое непостоянство я и увеличиваю тарифы вдвое. Так что, если хочешь его застать, то просто подожди.";
		...
		break;
        case "Step_2":
			dialog.text = "Я ждала тебя, "+ GetSexPhrase("красавчик","красотка") +". Аззи меня уведомил о том, что я должна кое-что сделать для тебя.";
		...
		break;
        case "Step_4":
			dialog.text = "И довольно неплохая, могу признаться... А ты кем Аззи приходишься, что он так печётся о тебе?";
			Link.l1 = ""+ GetSexPhrase("Знакомый","Знакомая") +" просто...";
			Link.l1.go = "Step_5";
		break;
        case "Step_5":
		...
		break;
        case "Step_6":
			dialog.text = ""+ GetSexPhrase("Даже и не мечтай, ты мне не интересен. Буду я шашни крутить со знакомым Аззи, когда есть САМ Аззи","Многого - это не разговор") +".\n"+
                     "В общем так, давай быстро и по делу. Я могу сделать тебя сильней, ловчей, повысить твоё восприятие, обаяние, выносливость или интеллект.\n"+
                     "Так как должна я поменять твоё тело?";
			Link.l1 = "Чтобы я "+ GetSexPhrase("был","была") +" сильней, ловчей, восприимчивей, харизматичней, умней - в общем, весь набор хочу.";
			Link.l1.go = "Step_7";
		break;
        case "Step_7":
			dialog.text = "Весь набор не получится.";
			Link.l1 = "Как это не получится?! "+ GetSexPhrase("Мало того, что доброму молодцу баньку не истопила, спать не уложила, она ещё и жульничает! Аззи сказал...","Аззи сказал...") +"";
			Link.l1.go = "Step_8";
		break;
        case "Step_8":
			dialog.text = "Слушай меня внимательно, "+ GetSexPhrase("добрый молодец","дорогуша") +"! Для того, чтобы сделать тебя сильней, мне нужно приладить к твоему телу новые большие мышцы.\n"+
                     ...
                     "И так далее по каждому пункту. Если всё сделать разом, то работать вообще ничего толком не будет. Исключением является удача - это как раз разумное сочетание различных характеристик тела, позволяющее выходить из различных сложных ситуации с честью. То есть, быть удачливым.\n"+
                     "Поэтому не дури, выбирай что-нибудь одно. Пока я добрая...";
			Link.l1 = "Понятно, значит я "+ GetSexPhrase("должен","должна") +" выбрать?";
			Link.l1.go = "Step_9";
		break;
        case "Step_9":
		...
		break;
        case "Charisma":
			dialog.text =  "Обаяние? Хм, хороший выбор, "+ GetSexPhrase("красавчик","красотка") +"! Ну что ж, готовим симпатичную мордашку с "+ GetSexPhrase("мужественными","неотразимыми") +" чертами лица - и будем менять. Полагаю, откладывать нет смысла, приступаем прямо сейчас.";
			Link.l1 = "Приступаем к чему?";
			Link.l1.go = "Step_10";
		...
		break;
        case "Luck":
			dialog.text =  "Хочешь быть "+ GetSexPhrase("удачливым, пират","удачливой, пиратка") +"? И правильно. Ну что ж, готовим всё новое - мышцы, связки, сухожилия, глаза, уши - в общем всё. Каждое не с выдающимим характеристиками, но очень даже ничего. Полагаю, откладывать нет смысла, приступаем прямо сейчас.";
			Link.l1 = "Приступаем к чему?";
			Link.l1.go = "Step_10";
		...
		break;
        case "Step_10":
			dialog.text =  "Как к чему? К операции, "+ GetSexPhrase("красавчик","красотка") +". Резать тебя будем! Ха-ха-ха!!";
			Link.l1 = "Что-то всё это мне не нравится...";
			Link.l1.go = "Step_11";
		break;
        case "Step_11":
			dialog.text =  "Да ты не бойся, боли не будет. Так что, быстренько тебе печень меняем, складочки подрезаем...";
			Link.l1 = "И что, я за этим ещё и наблюдать "+ GetSexPhrase("должен","должна") +"?!";
			Link.l1.go = "Step_12";
		break;
        case "Step_12":
			dialog.text =  ""+ GetSexPhrase("Экий ты корсар мнительный","Экая ты пиратка мнительная") +". Стольких людей на тот свет "+ GetSexPhrase("отправил","отправила") +", а своей крови боишься.";
			Link.l1 = "Слушай, ведьма, я ничего и никого не боюсь! Так что ты аккуратней в выражениях. А опасаюсь я за свой рассудок, умом тронуться можно - на такие вещи смотреть!";
			Link.l1.go = "Step_13";
		break;
        case "Step_13":
			dialog.text =  "Ну ладно, сделаем всё под наркозом, ха-ха-ха! Вот, выпей настой. Очнёшься ровно через два дня - будет твой заказ выполнен. И имей в виду, что впредь я знать тебя не знаю.";
			Link.l1 = "Эх, где наша не пропадала! Давай свою отраву. И осторожней там со мной...";
			Link.l1.go = "exit";
			AddDialogExitQuest("Azzy_PlusSkill");
		break;
        case "NotPosblAdd":
			dialog.text =  "Давай посмотрим, что ты имеешь... Нет, "+ GetSexPhrase("красавчик","красотка") +", достойной замены у меня нет. Извини, складские запасы ограничены, да и тебе грех жаловаться на то, что есть... Выбирай что-нибудь другое.";
			Link.l1 = "Хм, я "+ GetSexPhrase("польщён","польщена") +"... Так что, могу опять подумать над выбором?";
			Link.l1.go = "Step_9";
		break;
        case "Step_15":
		...
		break;
        case "Step_16":
			dialog.text =  "Не кричите на весь город, "+ GetSexPhrase("сударь","сударыня") +"! Аззи нельзя найти, лично я смогу его увидеть только в Вальпургиеву ночь. Я ничем не могу вам помочь.";
			Link.l1 = "Чёртова ведьма!";
			Link.l1.go = "exit";
		break;
        case "Step_17":
			dialog.text =  "Интересно послушать.";
			Link.l1 = "Я предлагаю тебе вызвать Аззи с помощью трёх хрустальных черепов тольтеков.";
			Link.l1.go = "Step_18";
		break;
        case "Step_18":
			dialog.text =  "Хм, я слышала об этих вещицах. Используются тольтеками для мистических ритуалов. Но для вызова демона нужен ещё один предмет - странная вещь.";
		...
		break;
        case "Step_21":
			dialog.text =  "Найдёшь - приходи, всё сделаем.";
			Link.l1 = ""+ GetSexPhrase("Понял","Поняла") +", буду искать.";
			Link.l1.go = "exit";
			pchar.questTemp.Azzy = "2DestrContract";
		break;
        case "Step_22":
			dialog.text =  "Итак, начнём! Ха-ха-ха (читает заклинание)";
