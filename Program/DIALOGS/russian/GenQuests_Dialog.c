	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			if(NPChar.id == "ContraMeetManQuest_Contrick_0") // Контрик в комнате по квесту "встреча в бухте"
			{
				...
				{
					dialog.text = "...Хм, и вправду, это он... Спасибо, "+ GetSexPhrase("парень","деваха") +", без твоей помощи нам пришлось бы туго - все пути за город нам заказаны. Вот, держи " + FindRussianMoneyString(iMoney) + " и можешь проваливать. А мы пойдём побеседуем с нашим товарищем...";
						link.l1 = "...";
			...
			{
				dialog.text = "Так-так-так... Что тут у нас?! Пираты?! Вам придётся пройти с нами в комендатуру для разбирательства...";
		...
		case "Hold_GenQuest50":
			...
			link.l1 = "Нет, этого не потребуется... Нужно просто передать " + pchar.GenQuest.Hold_GenQuest.RansomSum + " золотых в качестве выкупа.";
		...
		case "Hold_GenQuest60":
			...
			link.l1 = "Вот и великолепно. Сумма выкупа составляет " + pchar.GenQuest.Hold_GenQuest.RansomSum + " золотых.";
		...
		case "Hold_GenQuest71":
			...
			link.l1 = "Пираты требуют " + pchar.GenQuest.Hold_GenQuest.RansomSum + " золотых. Я долж"+ GetSexPhrase("ен","на") +" доставить деньги в порт, и тогда ваш друг будет свободен.";
		...
		case "Hold_GenQuest85":
			dialog.text = "И то ваша правда... Эх, была не была - за " + pchar.GenQuest.Hold_GenQuest.Sum + " золотых её отдам. Всё одно сам не решусь покойников тревожить.";
		...
		case "ShipWreck_13": // берём пассажирами
		...
		case "Convict_Begin":
			switch(sti(pchar.GenQuest.Convict.var))
			{
				...
				break;
				case 1:
					dialog.text = "Капитан, не одолжите несколько сотен пиастров? Мы бы и не просили, но нужда заставила.";
		...
		case "Convict_12_2":
			...
			link.l2 = "1000 золотых.";
			link.l2.go = "Convict_12_2_2";
			link.l3 = "3000 золотых.";
		...
		case "Convict_15":
			...
			if(Convict_CheckShipType(SHIP_TARTANE) || Convict_CheckShipType(SHIP_FELUCA))
			{
		...
		case "Convict_16_1":
			if(rand(1) == 0)
			{
				dialog.text = "А, чтоб тебя!.. Небось уже и патруль за нами послал"+ GetSexPhrase("","а") +"?! Это тебе даром не пройдёт!";
		...
		case "Convict_17_1":
			...
			{
				case 1:
					dialog.text = "Конечно-конечно! Какие могут быть вопросы! Вот, держите оговорённую сумму в золотых слитках и нашу безмерную благодарность.";
		...
		case "ReasonToFast_Lakey":
			...
			link.l2 = "Я прин"+ GetSexPhrase("ёс","есла") +" весточку от известных тебе людей.";
		...
		case "ShipEpidemy3":
			...
			link.l1 = RandPhraseSimple("Ну, ты мне за это ответишь!..","Я тоже удовлетвор"+ GetSexPhrase("ён","ена") +", что ты подохнешь раньше. И я это увижу...");
		...
		case "EncGirl_Lover1":
			...
			link.l1 = "Не знаю, насколько это важно для тебя, но я прив"+ GetSexPhrase("ёл","ела") +" " + pchar.GenQuest.EncGirl.name +". Она сбежала из дому и сейчас ждёт тебя недалеко от таверны.";
		...
		case "CaptainComission_22":
			...
			else // "операция 'Галеон'"
			{
				...
				dialog.text = "Передайте капитану патрульного " + GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(sti(pchar.GenQuest.CaptainComission.ShipType), "Name") + "Gen")) + " '" + pchar.GenQuest.CaptainComission.ShipTypeName + "' " + pchar.GenQuest.CaptainComission.Name + ", что ему нельзя появляться в " + XI_ConvertString("Colony" + pchar.GenQuest.CaptainComission.City + "Voc") + "... " +
							"Властям стали известны обстоятельства гибели " + GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(sti(pchar.GenQuest.CaptainComission.ShipTypeVictim), "Name") + "Gen")) + " '" + pchar.GenQuest.CaptainComission.VictimShipName + "'... нужно успеть, его ждёт арест... " + GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(sti(pchar.GenQuest.CaptainComission.ShipType), "Name"))) + " идёт в " + XI_ConvertString("Colony" + pchar.GenQuest.CaptainComission.City + "Acc") + " из " + XI_ConvertString("Colony" + pchar.GenQuest.CaptainComission.City1 + "Gen") + " и дней через 15 бросит якорь в порту... нужно постараться перехватить...";
		...
		case "CaptainComission_6_2":
			dialog.text = "Вчера я получил письмо от " + pchar.GenQuest.CaptainComission.PirateName + ", в котором он сообщил, что повышает сумму выкупа и грозится продать пленника в рабство. Покойный капитан должен был известить " + ChangeNameCase(NAMETYPE_MAIN, pchar.GenQuest.CaptainComission.PirateName, NAME_ACC) + ", что я согласен на прежние условия, и выдать аванс. Но теперь мне понятно, почему пираты повысили ставки.";
			link.l1 = "И что вы предлагаете?";
			link.l1.go = "CaptainComission_6_21";
		break;

		case "CaptainComission_6_3":
			dialog.text = "О-о, "+ GetSexPhrase("приятель","подруга") +", сдаётся мне, кто-то затеял нечестную игру. Предупреждаю сразу - это опасно и чревато неприятностями. У капитана " + pchar.GenQuest.CaptainComission.CapName + " было три сундука золота, которые он должен был передать " + ChangeNameCase(NAMETYPE_MAIN, pchar.GenQuest.CaptainComission.PirateName, NAME_DAT) + " в " + XI_ConvertString("Colony" + pchar.GenQuest.CaptainComission.PiratesCity + "Voc") + " в качестве выкупа за одного человека - " + pchar.GenQuest.CaptainComission.SlaveName + ". Раз он поручил тебе связаться с нами, то не мог не сказать о сундуках.";
		...
		case "CaptainComission_6_22":
			dialog.text = "Тогда держите деньги, и благослови вас Господь... Только нужно торопиться - боюсь, что " + pchar.GenQuest.CaptainComission.PirateName + " не станет ждать и продаст беднягу в рабство.";
		...
		case "CaptainComission_7":
			...
			if(pchar.GenQuest.CaptainComission.GetSlave == "GetSlave")
			{
				...
				{
					dialog.text = LinkRandPhrase("А-а, приветствую вас, капитан " + GetFullName(pchar) + ". Как успехи?","Ну что, капитан, вызволили нашего страдальца?","Рад вас видеть, капитан, в добром здравии. Как продвигается наше общее дело?");
					link.l1 = "Я прив"+ GetSexPhrase("ёз","езла") +" человека, о котором вы говорили. Он у меня на борту.";
		...
		case "CaptainComission_12_1":
			dialog.text = "Почему же нельзя? Полковник назначил за него цену в 200000 золотых, согласно прейскуранту.";
		...
		case "CaptainComission_301":
			...
			link.l1 = "Здравствуйте, я капитан " + GetFullName(pchar) +", действую по поручению некоего капитана " + pchar.GenQuest.CaptainComission.CapName +". Он просил передать, что в " + XI_ConvertString("Colony" + pchar.GenQuest.CaptainComission.City + "Dat") + " вас ждёт арест. Властям стала известна информация насчёт какого-то судна, если не ошибаюсь " + GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(sti(pchar.GenQuest.CaptainComission.ShipTypeVictim),"Name") + "Acc")) + ".";
		...
		case "CaptainComission_303":
			dialog.text = "Чёрт! Час от часу не легче... Послушайте, " + GetFullName(pchar) + ", я совсем не готов к такому повороту событий. Мы с капитаном " + pchar.GenQuest.CaptainComission.CapName + " условились встретиться в " + XI_ConvertString("Colony" + pchar.GenQuest.CaptainComission.City + "Dat") + ", чтобы загрузить к нему на борт кое-какой товар. Теперь же выясняется, что капитан погиб, а я не могу даже появиться в поселении, чтоб организовать вывоз.\nПохоже, у меня просто нет другого выхода, кроме как просить помощи у вас.";
		...
		case "CaptainComission_307":
			...
			link.l2 = "Нет, уважаемый. Во-первых: я не знаю, сколько товара придётся на мой трюм, а во-вторых: нет гарантии, что власти его не нашли и не вывезли, а у тайника не оставили засаду.";
...
//			bQuestDisableMapEnter = true;
		...
		case "CaptainComission_309":
			dialog.text = "Очень жаль, но вы не оставляете мне выбора. Одного " + GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(sti(pchar.GenQuest.CaptainComission.ShipType),"Name") + "Acc")) + " недостаточно, чтоб вывезти весь груз, ради которого я пожертвовал успешной карьерой и рискую жизнью. Да и ваша команда, в отличие от вас, наверняка будет не против помочь мне взамен на свою долю.";
			link.l1 = "И что вы предлагаете?";
		...
		case "CaptainComission_317":
			dialog.text = "Капитан, мы разыскиваем военного преступника " + pchar.GenQuest.CaptainComission.Name + ", укрывшего от властей ценный груз. Предъявите ваши документы.";
		...
		case "CaptainComission_321":
			...
			dialog.text = "Ещё одна просьба. Власти наверняка организуют погоню, вы не могли бы сопроводить моё судно до " + XI_ConvertString(pchar.GenQuest.CaptainComission.ConvoyShore + "Gen") + ", что на " + XI_ConvertString(GetIslandByCityName(sTemp) + "Voc") + "?";
		...
		case "CaptainComission_323":
			...
			dialog.text = "Это само собой разумеется. За эскорт я готов заплатить " + pchar.GenQuest.CaptainComission.ConvoyMoney + " золотых. Но при условии, что мы потратим не более " + pchar.GenQuest.CaptainComission.iDay + " дней. Это важно.";
		...
		case "CaptainComission_329":
			...
			dialog.text = "Тоже верно. Наличная сумма, скажем " + pchar.GenQuest.CaptainComission.GoodsSum + " золотых, избавит нас от необходимости конфликтовать с вашей командой. Обратите внимание - это стоимость половины нашего товара, так что всё справедливо... Соглашайтесь, и никто не пострадает.";
		...
		case "CaptainComission_330":
			...
			{
				link.l2 = "Похоже у меня нет выбора. Вот ваше золото.";
		...
		case "CaptainComission_Canoneer6":
			dialog.text = "По ошибке. По своей же ошибке уцелел - ирония судьбы. Я шнур в крюйт-камеру " + GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(sti(pchar.GenQuest.CaptainComission.ShipTypeVictim),"Name") + "Gen")) + " коротко отрезал, вот и не успела шлюпка далеко отойти, когда порох рванул. И ведь только меня взрывной волной и выкинуло, да ещё и контузило обломком фальшборта... Ребята в темноте не нашли, а как до берега добрался - действительно не помню.";
		...
		case "CaptainComission_Canoneer10":
			...
			dialog.text = "Не найдут. Я вход в пещеру взорвал так, что комар не пролезет, я ведь лучший пиротехник во всей эскадре. Если кто и найдёт - только контрабандисты, они каждую щель на острове знают...\n" +
				"Слушай, я давно голову ломаю, что с этим грузом делать, особенно после казни команды. В одиночку мне его ни продать, ни вывезти, тем более, что серьёзных дел с контуженым никто водить не будет... Я не хочу спекулировать на гибели товарищей, но за " + pchar.GenQuest.CaptainComission.Sum + " золотых готов показать вам тайник, при условии, что после загрузки вы доставите меня в поселение " + XI_ConvertString("Colony" + pchar.GenQuest.CaptainComission.CanoneerCity) + ".";
		...
		case "CaptainComission_370":
			...
			dialog.text = "Похоже, у меня сегодня приёмный день. Вы ко мне?";
		...
		case "CaptainComission_372":
			...
			link.l1 = "Капитан " + pchar.GenQuest.CaptainComission.CapName + " просил предупредить вас о том, что обстоятельства гибели " + GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(sti(pchar.GenQuest.CaptainComission.ShipTypeVictim),"Name") + "Acc")) + " '" + pchar.GenQuest.CaptainComission.VictimShipName + "' стали известны властям, " +
		...
		case "CaptainComission_374":
			dialog.text = "Ага... Вот, значит, в чём дело. Ну, теперь многое прояснилось... Итак, кто-то проболтался, что мы выпотрошили " + GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(sti(pchar.GenQuest.CaptainComission.ShipTypeVictim),"Name") + "Gen")) + ". " +
		...
		case "CaptainComission_377":
			...
			link.l1 = "Тогда действуем быстро и слажено: я открываю камеру, а вы следуете за мной. Не отставать и рот не открывать, пока не выберемся из форта.";
		...
		case "CaptainComission_385":
			dialog.text = "Мы здесь тоже не медуз ловим - разворачивай свою эскадру и уводи по ветру.";
		...
		case "CaptainComission_393":
			...
			dialog.text = "Капитан " + GetFullName(pchar) + ", мне несколько неловко такое предлагать, но не могли бы вы отдать мою долю золотом? Я думаю, " + pchar.GenQuest.CaptainComission.GoodsPrice + " золотых будет достойным компромиссом.";
		...
		case "CaptainComission_398":
			sld = characterFromId("Blaze");
			dialog.text = "Э, нет, капитан, это уже твои проблемы! Ведь я сразу предлагал уводить " + GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(sti(pchar.CaptainComission.ShipTypeVictim),"Name"))) + " в укромное место, а ты всё за свою карьеру хватался. Вот и дождался, пока твой любимый губернатор не упёк тебя в тюрьму." +
			...
			link.l1 = "Послушай, боцман, ты разве забыл что у " + GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(sti(pchar.CaptainComission.ShipTypeVictim),"Name") + "Acc")) + " был разбит грот? А про крен до самого фальшборта ты тоже забыл? А ты хоть понимаешь, что, если бы я не отдал "+ NPCharSexPhrase(sld, "этому человеку","ей") +" половину, то ты скорее бы снега дождался, чем товара?";
		...
		case "CaptainComission_399":
			...
			link.l1 = "Вижу, ты совсем потерял голову, наверное звон пиастров уже стоит в твоих ушах.";
		...
		case "ContraDeliverQuest_1":
			...
			else // Деньги или патруль
			{
				if(rand(2) == 1) // Патруль
				{
					dialog.text = "Спасибо за доставку. Теперь можешь прова... Хм, кто-то идёт...";
						link.l1 = "Что?!";
...
//				link.l1.go = "ChurchGenQuest1_DialogShip_5_2_1"; // Сваливаем, поверили ему
		...
		case "Church_GenQuest_2_ShoreBandit_4":
			dialog.text = "Эй, полегче на поворотах, судар"+ GetSexPhrase("ь","ыня") +"!.. Чёрт, я же просил этого олуха не поднимать шума раньше времени...";
		...
		case "Church_GenQuest_2_ShoreBandit_Real_Band_2_3":
			...
			{
				link.l1 = "А вот здесь ты промахнулся, алчная крыса - за мой счёт поживиться не удастся!"+ GetSexPhrase(" Я вольный ветер, сегодня - здесь, завтра - там, сегодня нищ, завтра богат...","") +"";
		...
		// Берём их в пассажирами - держать курс на населённый остров
		...
		// Базар с главарём уже в другой бухте на обитаемом острове
		case "PiratesOnUninhabited_12":
			...
			{
				// Требуют денег
				dialog.text = "Капитан, не сочтите за наглость, но мы несколько поиздержались. Шутка ли потерять всё. Не могли бы вы ссудить нам пару горстей пиастров на первое время? Мы обязательно их вернём... при случае...";
				...
				link.l2 = "Однако же... И сколько вы хотите пиастров?";
		...
		case "PiratesOnUninhabited_17":
			...
			dialog.text = "Ну скажем, по " + iMoney + " золотых монет на брата, нас бы вполне устроило... Если конечно капитан не считает, что это слишком малая компенсация, за те лишения, которые выпали на нашу долю... хе-хе...";
		...
		case "PiratesOnUninhabited_21":
			...
			{
				// Игрок врёт - свободное место есть
				dialog.text = "Ну, что Вы, капитан, как можно?! Мы обязательно поставим за Вас свечку... Если когда-нибудь доберёмся до церкви...";
				link.l1 = "Будем надеяться...";
		...
		// Базар с главарём уже в другой бухте на обитаемом острове. Вариант, когда нам обещали карту сокровищ
		...
		case "PiratesOnUninhabited_39":
			dialog.text = "Ах старая каналья! Опять решил нас надуть! Ну, это у него не пройдёт. Передай, что золото он сможет забрать только вместе с нами!";
			link.l1 = "Он велел без золота не возвращаться и на борт никого не брать...";
			link.l1.go = "PiratesOnUninhabited_40";
		break;

		case "PiratesOnUninhabited_40":
			dialog.text = "Раз уж ты ему так веришь, то можешь оставаться здесь. А мы заберём твой корабль, прихватим золото и отправимся " + LowerFirst(XI_ConvertString(PChar.GenQuest.PiratesOnUninhabited.StartShore + "Acc")) + ". Нам очень хочется взглянуть в лицо " + PChar.GenQuest.PiratesOnUninhabited.MainPirateName + ".";
		...
		case "PiratesOnUninhabited_43":
			...
			{
				link.l1 = "Хорошо, давайте в команду. Только гляди мне, - не баловать! У меня с этим строго!";
				link.l1.go = "PiratesOnUninhabited_9"; // Берём в команду
			}
		...
		case "PiratesOnUninhabited_51":
			...
			link.l1 = "Было бы лучше, если бы он утопил меня? Видишь, сам"+ GetSexPhrase("","а") +" еле корму унёс"+ GetSexPhrase("","ла") +"?";
		...
		case "PiratesOnUninhabited_52":
			...
			else
			{
				...
				{
					link.l2 = "Чёрт бы тебя побрал с твоим кораблём вместе! Держи свои пиастры и не попадайся мне больше на глаза!..";
		...
		case "Greedy":
			dialog.text = "О, ты тут камешек драгоценный наш"+ GetSexPhrase("ел","ла") +"? Отдай его пожалуйста. С тебя не убудет, а нам поможет штаны поддержать.";
		...
		case "Greedy_3":
			dialog.text = "Ну отдай. Мы тебе ещё пригодимся. Мы ведь и правда едва концы с концами сводим, а твой кошелёк от этого ощутимо не потяжелеет. Скряга этот, ростовщик наш - ещё каких поискать.";
...
void PiratesOnUninhabited_SetCapToMap()
{
	...
	Map_CreateTrader(character.fromShore, character.toShore, "PiratesOnUninhabited_BadPirate", GetMaxDaysFromIsland2Island(GetArealByCityName(character.toCity), GetArealByCityName(character.fromCity)) + 1);
