...
void ProcessDialogEvent()
{
	...
	case "WantToRemove":
		...
		if (Npchar.alignment == "good")
		{
			...
			if (sti(Pchar.money) >= sti(NPChar.rank)*500)
			{
				Link.l2 = "Я давно знал"+ GetSexPhrase("","а") +", что ты уйдёшь... такая служба не для тебя. Но упрекнуть мне тебя не в чём. Вот "+sti(NPChar.rank)*500+" золотых - пригодятся, пока найдёшь работу по душе.";
			...
			if (sti(Pchar.money) >= sti(NPChar.rank)*500)
			{
				Link.l2 = "Я знал"+ GetSexPhrase("","а") +", что ты уйдёшь... такая служба не для тебя. Но упрекнуть мне тебя не в чём. Вот "+sti(NPChar.rank)*500+" золотых - пригодятся, пока будешь искать новое место.";
				Link.l2.go = "Im_kind_A2";
			}
		...
		case "Node_2":
			dialog.text = NPCharRepPhrase(Npchar, RandSwear() + "Да я лучший "+ XI_ConvertString(Npchar.quest.officertype) + " в этих водах. " + Npchar.quest.officertype_2 + "Готов оказать вам услугу, пойдя на службу.",
                                    "Ну, говорят, что я неплохой " + XI_ConvertString(Npchar.quest.officertype) + ". " + Npchar.quest.officertype_2 + "Возьмите меня на своё судно, капитан - убедитесь в этом сами.");
		...
		case "price":
			dialog.text = "Думаю, мы с вами сойдёмся на " + Npchar.quest.OfficerPrice + " пиастров.";
		...
		case "Get_out_A1_peace":
			...
			Link.l1 = "Стой. Сейчас я не могу оставить судно без офицера. Но позднее мы ещё вернёмся к этому разговору.";
			Link.l1.go = "exit_bad";
