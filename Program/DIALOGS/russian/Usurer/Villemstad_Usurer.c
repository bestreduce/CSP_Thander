...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
    ...
    switch (Dialog.CurrentNode)
	{
		case "quests":
            ...
            {
                link.l1 = "Я "+ GetSexPhrase("хотел","хотела") +" бы попросить у вас помощи в одном деле. Эта помощь будет щедро оплачена.";
                link.l1.go = "Step_E4_1";
            }
			if (pchar.questTemp.State == "Intelligence_Curacao_NoMoney_Yozef")
            {
                link.l1 = "Я "+ GetSexPhrase("хотел","хотела") +" бы поговорить о том деле на 100000 монет.";
                link.l1.go = "no_money";
            }
            if (pchar.questTemp.State == "Inquisition_toCuracao" || pchar.questTemp.State == "Inquisition_afterFrancisco" )// квест №2 голл. линейки.
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            dialog.text = "Что, ты ещё не все мне сказки "+ GetSexPhrase("рассказал","рассказала") +" про прощение Папы?";
                			link.l1 = "Ладно, Йозеф, поговорим ещё...";
                			link.l1.go = "exit";
                        }
                        else
                        {
                            dialog.text = "Что ещё тебе нужно, "+ GetSexPhrase("проклятый кровопийца, иезуитский прихвостень","проклятая кровопийца, иезуитская прихвостня") +" ...";
...
//********************** Английская линейка, квест №4 "Разведка в Кюрасао" ************************
		case "Step_E4_1":
			...
			link.l1 = "Я "+ GetSexPhrase("готов","готова") +" дорого заплатить за информацию. Мне нужно знать о планах генерал-губернатора Кюрасао в отношении англичан.";
			link.l1.go = "Step_E4_2";
		break;
		case "Step_E4_2":
			dialog.text = "Надеюсь, вы понимаете, о чем просите.";
			link.l1 = "Надеюсь, и вы понимаете, что я буду очень "+ GetSexPhrase("щедр","щедра") +".";
			link.l1.go = "Step_E4_3";
		break;
		case "Step_E4_3":
			dialog.text = "100000 монет и я расскажу вам все, что знаю.";
			link.l1 = "Я "+ GetSexPhrase("готов","готова") +" внести требуемую сумму.";
		...
		case "agree_to_pay":
            if (sti(pchar.money) >= 100000)
            {
        		dialog.text = "Что ж, могу вам доложить следующее: около часа тому назад в резиденции Питера Стэвезанта, генерал-губернатора этого острова, закончился военный совет по поводу как раз вашего вопроса - участие голландских колоний в торговой войне с Англией. На самом совещании я не присутствовал, но знаю, что по результатам оного вестовой генерал-губернатора отправляется куда-то по срочному поручению. На рейде вестового ожидает бриг.\n"+
                         "Вы по виду пират, и вам не составит труда взять голландский бриг. Так что поторопитесь, пока тот не скрылся за горизонтом. И ещё... я вас не знаю. Прощайте.";
            ...
            {
        		dialog.text = "Хмм, похоже таких денег у вас всё-таки нет. Впредь, прошу вас, не морочьте мне голову. Жду вас с требуемой суммой.";
        		link.l1 = "Хорошо, я вас "+ GetSexPhrase("понял","поняла") +".";
        		link.l1.go = "exit";
		...
		case "no_money":
			...
			link.l1 = "Да, деньги у меня и я "+ GetSexPhrase("готов","готова") +" их вам вручить. Так что же вам удалось узнать?";
			link.l1.go = "agree_to_pay";
			link.l2 = "Нет ещё, но я обязательно достану их и вернусь позже.";
			link.l2.go = "no_money_now";
			link.l3 = "Не слишком ли жирно, грязная свинья?";
			link.l3.go = "fack_yozef";
		break;
		case "fack_yozef":    // посыл Йозефа с попыткой замочить
			dialog.text = "Да ты "+ GetSexPhrase("нахал","нахалка") +"! Попрошу убраться из моего дома немедленно!";
			link.l1 = "Чёрт тебя побери, проклятый кровопийца! Сейчас я тебе выпущу кишки!";
			link.l1.go = "Step_E4_4";
		break;
		case "Step_E4_4":
			dialog.text = "Не тут то было, "+ GetSexPhrase("ублюдок","дрянь") +". Охрана, ко мне!!!";
			link.l1 = "Проклятье!";
...
//********************** Испанская линейка, квест №2 "Инквизиция" ************************
		case "Step_S2_1":
			...
			link.l1 = "Это вы так считаете, а орден другого мнения. Так что деньги вам придётся заплатить.";
			link.l1.go = "Step_S2_2";
		break;
		case "Step_S2_2":
			dialog.text = "Я вам ничего платить не буду! И если вы сейчас же не уберётесь из моего дома, то я позову слуг!";
		...
		break;
		case "Step_S2_5":
			dialog.text = "И с чего это я должен выслушивать бред, который ты несёшь?";
		...
		case "Step_S2_6":
			...
			link.l1 = "А такой гарантии нет. Побудь хоть немного в шкуре тех, кого ты доводил до отчаяния. Узнай, почём фунт лиха. И хватит болтать, давай деньги.";
...
//********************** Пиратская линейка, квест №5 ОГЗ ************************
		case "Step_P5_1":
			dialog.text = "Нет. Не надо.\nХе, вы, так называемые вытрясыватели долгов, как близнецы все, "+ GetSexPhrase("даже речь у вас одинаковая","даже речь у вас одинаковая. Хоть вы и девушка") +". Вот и Эйвори точно так же разговаривает.";
			link.l1 = "Джон Эйвори?! Так это мой приятель. Он ещё в городе? Где его можно отыскать?";
			link.l1.go = "Step_P5_2";
		break;
		case "Step_P5_2":
			dialog.text = "Понятия не имею. Он вольный человек, ходит, где заблагорассудится. А насчёт того, что он ещё в Виллемстаде, это вряд ли, здесь для него работы не осталось. Вы же "+ GetSexPhrase("приятель","приятельница") +" Джона, должны понимать, что искать его надо там, где ром льётся рекой - в тавернах!";
		...
		case "Step_P5_3":
			...
			dialog.text = "Сказал, что теперь к англичанам собирается. Вроде о " + XI_ConvertString("Colony" + pchar.questTemp.piratesLine.Q5.city_1 + "Voc") + " шла речь, но точно не скажу.";
