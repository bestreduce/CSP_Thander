void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		//ОЗГ
			...
			if (!CheckAttribute(pchar, "questTemp.Headhunter") && pchar.rank >= 10)
			{
...
//navy <-- */
		...
		case "Give_vector_1":
			dialog.text = "Нет. Он в последнее время стал очень скрытен. Обратитесь к нему - он сам вам всё расскажет.";
			link.l1 = "Хорошо, я обязательно загляну к нему.";
			link.l1.go = "Give_vector_2";
			link.l2 = "Хм... Знаете, у меня у само"+ GetSexPhrase("го","й") +" слишком много проблем, чтобы решать ещё и чужие. Идти на Мартинику, даже не зная того, что именно вашему другу от меня нужно? Извините, но это не для меня.";
