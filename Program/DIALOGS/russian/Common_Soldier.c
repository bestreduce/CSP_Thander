...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
			...
			else
			{
				...
				if (GetNationRelation(sti(NPChar.nation), GetBaseHeroNation()) == RELATION_ENEMY && sti(NPChar.nation) != PIRATE)
				{
					...
					if (CheckNationLicence(sti(npchar.nation)))
					{
						...
						{
							link.l2 = "Я бросил"+ GetSexPhrase("","а") +" якорь у " + XI_ConvertString(GetIslandByCityName(npchar.city)+"Gen") + " под флагом " + NationNameGenitive(sti(pchar.nation)) + ". Что тебе ещё не ясно?";
						}
					...
					else
					{
						...
						if (findsubstr(pchar.location.from_sea, "_town" , 0) != -1) //если причалил в городе
						{
							link.l1 = "Ты что, не видишь, как на моём корабле развевается флаг " + NationNameGenitive(sti(pchar.nation)) + "?!";
						}
						else //если причалил не в городе
						{
				...
				else
				{
					if (sti(NPChar.nation) == PIRATE)
					{
						if (sti(rColony.HeroOwn) == true) // наш горожанин
						{
			         		switch (rand(10))
							{
								...
								case 3:
									dialog.text = "Спешу пожаловаться, капитан: мы все стремимся снова в море. Эта служба на суше просто поперёк горла стоит!";
								...
								case 9:
									dialog.text = "Скажу вам по секрету, капитан, поскольку вы нас никогда не обижали, завалили мы вчера с ребятами здесь одну тёлочку...";
					...
					if (pchar.questTemp.MC == "toCaracas" && npchar.city == "Caracas")
					{
						dialog.text = LinkRandPhrase("Видите, что творится в городе? Но мы не будем ни во что вмешиваться...",
							...
							"Мы ведём себя так, как будто в городе всё нормально. Нельзя поддаваться настроениям толпы, иначе волнения могут перейти в бунт!");
						link.l1 = "Понятно...";
						link.l1.go = "exit";
						break;
					}
					...
					{
						dialog.text = "Вы ходили в таверну. И что скажете?";
						link.l1 = "Пока ничего...";
						link.l1.go = "exit";
						break;
					}
					...
					switch (rand(10))
					{
						...
						case 9://///////////////////////////////////////////
							...
							link.l1 = ""+ GetSexPhrase("Соблазнительно, чёрт побери, но я вынужден отказаться. Казарменная муштра не для меня.","Спасибо за комплимент, солдат, мне очень приятно.") +".";
							link.l1.go = "exit";
						break;
		...
		{
			dialog.text = RandPhraseSimple("Сдаётся мне, что это обман... Давай-ка пройдём в комендатуру, "+ GetSexPhrase("голубчик","голубушка") +", там разберёмся...", "Хм, что-то подсказывает мне, что ты не "+ GetSexPhrase("тот","та") +", за кого себя выдаёшь... \nНемедленно сдай оружие, " + GetAddress_Form(npchar) + ", и следуй за мной для дальнейшего разбирательства!");
			...
			if(bBribeSoldiers == true)
			{
				...
				{
					link.l2 = RandPhraseSimple("Тут рядом кошель валялся, случаем не ваш? Ух, а он тяжёлый, зараза! Вот, держите и больше не теряйте. Кстати, я могу идти?", "Вы меня раскусили. На самом деле я просто странствую по Карибам и раздаю деньги нуждающимся. Говорят, страже мало платят? Это необходимо исправлять. Начнём с вас. Вот, держите. Теперь всё в порядке?");
		...
		case "SeekSpy_Checking":
			...
			link.l1 = RandPhraseSimple("Я "+ GetSexPhrase("понял","поняла") +". Спасибо за помощь.", "Всё ясно. Спасибо за помощь.");
