...
// ============================================================================
	switch(Dialog.CurrentNode)
	{
		...
		break;
		case "SharpPearl_2":
			dialog.text = "Понятия не имею. А вообще этот лучший корсар всех времён часто ошивается в борделях. Там и ищи...";
		...
		//********************* Английская линейка *********************
 		...
 		case "Step_1_4":
			dialog.text = "Мне кажется, я ясно выразился, что не могу провести сейчас данную операцию. Однако если вы найдёте меня несколько позже в моём доме на Ямайке, то, возможно, я смогу предложить лично вам горячее дело. Дело для настоящего корсара. А пока всего хорошего.";
	 	...
	 	case "Step_3_1":
            ...
            {
    			dialog.text = "Хе-хе, время пришло ещё раньше! Когда ты был"+ GetSexPhrase("","а") +" у меня и уговаривал"+ GetSexPhrase("","а") +" напасть на Кюрасао, я уже планировал экспедицию и начал подготовку к ней. Но не на Кюрасао.";
	 	...
	 	case "Step_3_3":
			...
			link.l1 = "Теперь ясно. Если не возражаешь, давай вернёмся к нашим делам. Что за горячее дело?";
			link.l1.go = "Step_3_4";
  		break;
	 	...
	 	case "Step_3_4":
			...
			link.l2 = "Чёрт! Как заманчиво это ни звучит, но я вынужден"+ GetSexPhrase("","а") +" отказаться. Состояние моего корабля оставляет желать лучшего, да и команда на пределе после длительной болтанки по Карибам. Не в этот раз.";
			link.l2.go = "Step_3_6";
  		break;
	 	...
	 	case "I_want_attack":    // Идём на испанцев, а может и не идём
            if (GetCompanionQuantity(pchar) == 1)   // Заставляем геймера избавиться от взятых кубиков.
            {
           		dialog.text = "Ну что ж, атакуемый город - Белиз. Мне донесли, что сейчас там собрано много золота с испанских золотых шахт на американском континенте. Золото готовится к отправке в Европу, нам нельзя медлить.\n"+
                              ...
                              "И ещё один важный момент - если ты будешь штурмовать форт и тебе улыбнётся удача, то знай, что команды наших кораблей поддержат тебя при штурме.\n"+
            ...
            else
            {
                NextDiag.TempNode = "Step_3_4";
               	dialog.text = "За тобой целый хвост тянется! Мы идём на серьёзное дело, нам обуза из лишних кораблей ни к чему."+
	 	...
	 	case "Have_hot_bussines":    // Если геймеру вдруг захочется ещё пообщаться, не знаю, сможет ли. Но на всякий случай.
			dialog.text = "Чего мы ждём? Время дорого, нужно выступать.";
        ...
        break;
	 	...
	 	case "Step_3_10":
            ...
            {
                dialog.text = "Подожди ещё минуту. Я тут подумал: может быть, весьма кстати, что ты не хочешь обременять себя куплей-продажей? Если так, то я готов прикупить у тебя твою долю золота.\n"+
            ...
            else
            {
                dialog.text = "Подожди минутку. Смотри, твой корабль просел ниже ватерлинии. У тебя перегруз, ты не сможешь далеко уйти из порта.\n"+
                              "Максимальная грузоподъёмность твоего корабля " + GetCargoMaxSpace(pchar) + " центнера, а в трюмах у тебя сейчас " + RecalculateCargoLoad(pchar) + " центнера\n"+
        ...
        break;
	 	...
	 	case "sale_gold_25_persent":
			...
			link.l1 = "Бывай, Морган. Ещё увидимся.";
			link.l1.go = "out_fort";
        ...
        break;
	 	...
	 	case "sale_gold_all":
			dialog.text = "Ты продаёшь мне все? Великолепно! Выгодная сделка для нас обоих. Прощай.";
			link.l1 = "Бывай, Морган. Ещё увидимся.";
			link.l1.go = "out_fort";
        ...
        break;
 	 	case "Step_H3_3":
			dialog.text = "Ну что же, тогда давайте сразу к делу. Мэнсфилд мне - как отец родной, ради него я готов на многое. Но вот ради Голландии я на многое не готов. Поэтому если хотите, чтобы я вам помог, придётся вам поработать лично на меня.";
        ...
        break;
 	 	case "Step_H3_6":
            if (CheckAttribute(pchar, "questTemp.Talks.Tavern") && CheckAttribute(pchar, "questTemp.Talks.Store") && CheckAttribute(pchar, "questTemp.Talks.Shipyard") && CheckAttribute(pchar, "questTemp.Talks.Usurer") && CheckAttribute(pchar, "questTemp.Talks.Brothel"))
 	 	    {
    			dialog.text = "Ну что же, я полагаю, все ясно. Такой суммой Пикардиец не располагал ранее и не получил в результате проведённой совместно операции. Учитывая мои подозрения, ему придётся серьёзно попотеть, дабы обосновать такие траты... Я благодарю вас.";
            ...
            {
                dialog.text = "Понятно. Но это ещё ничего не доказывает точно. Я знаю, что такие суммы у него есть в наличии. Так что вам придётся вернуться на Тортугу и продолжить сбор информации.";
    			link.l1 = "Я узнал"+ GetSexPhrase("","а") +" достаточно много, чтобы сделать соответствующие выводы.";
    			link.l1.go = "Step_H3_7";
    			link.l2 = "Хорошо, ещё раз побываю на Тортуге и продолжу расспросы.";
    			link.l2.go = "exit";
    		}
        break;
 	 	case "Step_H3_7":
        ...
        break;
 	 	case "Step_H6_2":
			...
			link.l1 = "Мы нападём на Сантьяго?";
			link.l1.go = "Step_H6_3";
        break;
 	 	case "Step_H6_3":
			dialog.text = "Возможно. Однако нам не стоит торопиться - уже выстроилась очередь за головой генерал-губернатора Сантьяго. Англичане тоже хотят его крови, а вы с Англией в войне и совместные действия невозможны. Посему эта партия должна быть чётко расписана по ролям.";
        ...
        break;
 	 	case "Step_H6_6":
			dialog.text = "Отлично! Ну что же, ты сделал"+ GetSexPhrase("","а") +" своё дело, спасибо. Ваш весомый вклад в это святое дело будет по достоинству оценен Береговым братством.";
			link.l1 = "Я очень рад"+ GetSexPhrase("","а") +", что мы смогли помочь вам. Ну, а как же штурм города?";
			link.l1.go = "Step_H6_7";
        break;
 	 	case "Step_H6_7":
			dialog.text = "Это уже не ваша задача, вы своё дело сделали. Мэнсфилд будет отомщен.";
		...
		//********************* Французская линейка *********************
        case "FraLine8_talk":
            dialog.text = "Ага, да это сам"+ GetSexPhrase("","а") +" капитан " + GetFullName(pchar) + "! Я давно за тобой присматриваю. До сих пор ты не лез"+ GetSexPhrase("","ла") +" в мои дела, и поэтому ещё жив"+ GetSexPhrase("","а") +" и здоров"+ GetSexPhrase("","а") +". Говори, зачем пожаловал"+ GetSexPhrase("","а") +". Полагаю, не для праздных разговоров.";
		...
		case "PL_Q1_1":
			dialog.text = "Да-а-а, "+ GetSexPhrase("парень","деваха") +", попал"+ GetSexPhrase("","а") +" ты в переплёт. Молодец - сопротивляться не стал"+ GetSexPhrase("","а") +".\nХорошо, что в Порт Рояле у меня много друзей, и комендант мне сразу о тебе доложил. Так что вопрос о твоём освобождении уже решен.";
			link.l1 = "А как же Джон Болтон? Я же к тебе его вёз"+ GetSexPhrase("","ла") +".";
			link.l1.go = "PL_Q1_2";
		break;
		case "PL_Q1_2":
		...
		case "PL_Q2_1":
			...
			dialog.text = "А-а-а, приш"+ GetSexPhrase("ел","ла") +". Это хорошо. Кое-что я уже выяснил по Болтону.\nСейчас тебе нужно отправиться на Мартинику. Там живёт один проходимец, зовут его Эдвард Лоу. Как его разыщешь, вручи от меня Чёрную Метку.";
		...
		break;
		case "PL_Q2_awaywait":
			dialog.text = "Эдвард Лоу мёртв?";
			link.l1 = "Нет ещё, Морган, но скоро будет.";
			link.l1.go = "exit";
		...
		case "PL_Q3_notFound":
			dialog.text = "Что-то у тебя вид невесёлый, никак упустил"+ GetSexPhrase("","а") +" этого Лоу?";
			link.l1 = "Упустил"+ GetSexPhrase("","а") +"...";
			link.l1.go = "PL_Q3_notFound_1";
		break;
		case "PL_Q3_notFound_1":
			dialog.text = "Что ж, почему-то я так и подумал. Может, и не стоило тебе такие поручения давать, может, твоя стихия - налёты, абордажи, схватки?\nВремя покажет. Ступай, возможно, когда-нибудь, я дам тебе ещё один шанс.";
		...
		case "PL_Q4_1":
			dialog.text = "Есть. Мне как раз нужен кто-то вроде тебя. Сразу предупреждаю, работы тут немного, зато доход неплохой. Составим конкуренцию лучшему пирату всех времён - мистеру Бартоломью Шарпу? Короче, у меня появилась информация, где будут находиться тартаны ловцов жемчуга ближайший месяц.";
			link.l1 = "Деньги на дороге не валяются. И где же это?";
			link.l1.go = "PL_Q4_3";
		break;
		// Warship 13.07.09 В новом варианте нету
		/*case "PL_Q4_2":
			dialog.text = "Хм, в данном случае все достаточно просто. Составим конкуренцию лучшему пирату всех времён - мистеру Бартоломью Шарпу? Короче, у меня появилась информация, где будут находиться тартаны ловцов жемчуга ближайший месяц.";
		...
		break;
		case "PL_Q4_4":
			dialog.text = "По моим расчётам, у них к тому времени должно быть не менее тысячи малых и пятисот больших жемчужин. Так что постарайся. Добыча – пополам.";
		...
		case "PL_Q4_badWork":
			...
			link.l1 = "Плохо. Эти чёртовы тартаны рассыпались от меня кто куда. Ни одну не догнал"+ GetSexPhrase("","а") +".";
		...
		case "PL_Q4_goodWork_1":
			...
			else
			{
				...
				{
					dialog.text = "Отлично, ты хорошо поработал"+ GetSexPhrase("","а") +". К тому же, выполнил"+ GetSexPhrase("","а") +" 'план по добыче', – жемчуга не меньше означенного количества. Молодец, что ещё я могу сказать!";
					link.l1 = "А разве могло быть иначе?..";
					link.l1.go = "PL_Q4_goodWork_3";
				}
				else
				{
					dialog.text = "Гм, не могу сказать, что я доволен. Ты не уложил"+ GetSexPhrase("ся","ась") +" в означенный план - 1000 мелких и 500 крупных жемчужин. Так что извини, но я забираю всё себе...";
					link.l1 = "Чёрт возьми! Но я же выполнил"+ GetSexPhrase("","а") +" работу!";
		...
		case "PL_Q4_goodWork_3":
			dialog.text = "Половина добычи - твоя, как и договаривались. Остальное - моё.";
		...
		case "PL_Q4_goodWork_4":
			dialog.text = "Не забывай заглядывать ко мне. Может ещё что-нибудь подвернется.";
		...
		case "PL_Q4_goodWork_5":
			dialog.text = "Ты выполнил"+ GetSexPhrase("","а") +" дело плохо, "+ GetSexPhrase("приятель","подруга") +"! Я рассчитывал на совсем другой доход. И если ты проморгал"+ GetSexPhrase("","а") +" тартаны - это за счёт твоей доли. В общем, спорить бесполезно, жемчуг я забираю...";
		...
		case "PL_Q5_begin_1":
			...
			link.l1 = "Да, это правда. У меня был заказ на него. А в чём, собственно дело?";
		...
		break;
		case "PL_Q5_begin_6":
			dialog.text = "Ладно. Оливер, прикажи немедленно доставить сюда Гудли! Разберёмся на месте, так сказать... А ты пока постой в сторонке.";
		...
		case "PL_Q5_talking_2":
			...
			link.l1 = "Адмирал, это недоразумение! Меня оклеветали! Ты же меня столько лет знаешь, неужели ты поверишь "+ NPCharSexPhrase(sld, "этому молокососу","этой девке") +"?! Да я "+ NPCharSexPhrase(sld, "его","её") +" собственными руками!";
			link.l1.go = "exit";
		...
		case "PL_Q5_afterFight_1":
			dialog.text = "Буду откровенен: я НЕ знаю, что было бы, не проведи я расследование. Ладно, не расстраивайся. Ты всё равно врать не умеешь. Это даже идиоту понятно.\nДа, это ещё не все, отправляйся к Джекмену. У него там случилась какая-то неприятность, в которой снова фигурирует твоё имя. Заодно расскажешь о сегодняшнем событии. Похоже, что это звенья одной цепи.";
		...
		break;
		case "PL_Q7_begin_1":
			dialog.text = "Если ты хочешь рассказать историю про "+ GetSexPhrase("своего двойника","свою сестрёнку") +", то она мне уже известна.";
		...
		case "PL_Q7_begin_3":
			...
			link.l1 = "И что за человек? Гхм... надёжный?";
		...
		case "PL_Q7_LaVegaLate_2":
			...
			link.l1 = "Хорошо, сделаю всё в лучшем виде.";
			link.l1.go = "exit";
		...
		case "PL_Q7_LeonNotFound_2":
			dialog.text = "М-да, видимо, Стива мы уже не увидим... Ну что же, заходи ко мне как-нибудь. Может, подвернётся для тебя какая работа.";
		...
		case "PL_Q7_afterLeon":
			...
			link.l1 = "К сожалению, Стив Линней мёртв. Его убили пираты с фрегата 'Леон'. Но с ними я уже рассчитал"+ GetSexPhrase("ся","ась") +".";
			link.l1.go = "PL_Q7_afterLeon_1";
		break;
		case "PL_Q7_afterLeon_1":
			dialog.text = "Карамба! Жаль Стива...";
			link.l1 = "Это ещё не все. Кэп этого фрегата уговаривал меня работать на адмирала Берегового Братства.";
		...
		case "PL_Q7_afterLeon_3":
			dialog.text = "Ха! Хотя чему удивляться? Зависть, алчность и тщеславие – вот три кита, на которых держится мир. Капитан пиратской шхуны - и тот каждую минуту рискует получить чёрную метку от своей команды, стоит лишь одному из его пиратов возомнить себя умнее остальных. А уж адмирал Берегового Братства...";
		...
		case "PL_Q7_afterLeon_5":
			dialog.text = "Значит так. Ходят слухи, что Соукинс планирует акцию. Нельзя допустить, чтобы его затея удалась, иначе он приобретёт дополнительный вес. Он удачлив, а это нужно исправить.";
		...
		case "PL_Q7_GoodWork_4":
			dialog.text = "Чёрт! И даже не узнал"+ GetSexPhrase("","а") +" почему он был так ценен для Соукинса?";
		...
		case "PL_Q8_1":
			...
			{
				dialog.text = "Хм, дошли до меня слухи, что ты, "+ GetSexPhrase("приятель","подруга") +", связал"+ GetSexPhrase("ся","ась") +" с " + NationNamePeopleAcc(sti(pchar.questTemp.NationQuest)) + ". Я тебе, кажется, ясно говорил, чтобы ты закончил"+ GetSexPhrase("","а") +" работу на власти, а потом уже меня дёргал"+ GetSexPhrase("","а") +". Что тебе непонятно?!";
				link.l1 = "Все ясно, Генри, извини...";
				link.l1.go = "exit";
			}
		break;
		case "PL_Q8_2":
			dialog.text = "Дело? Хм, дело серьёзное. Я собираю всех пиратов для грандиозного предприятия, которое оставит след в истории!";
			link.l1 = "Серьёзное заявление.";
			link.l1.go = "PL_Q8_3";
		break;
		case "PL_Q8_3":
			dialog.text = "Более чем! И в этом деле, "+ GetSexPhrase("друг мой","моя дорогая") +", у тебя будет особая миссия. Пришла пора свести счёты с нашим другом Соукинсом.";
			link.l1 = "Давно пора.";
			link.l1.go = "PL_Q8_4";
		break;
		case "PL_Q8_4":
		...
		break;
		case "PL_Q8_10":
			dialog.text = "Не уверен, поэтому мы пойдём не через мыс Горн, а через перешеек.";
			link.l1 = "Дьявол! Это же ещё большее безумие!!";
			link.l1.go = "PL_Q8_11";
		break;
		case "PL_Q8_11":
		...
		case "PL_Q8_12":
			...
			link.l1 = "Понятно. Что мне ещё нужно знать?";
			link.l1.go = "PL_Q8_13";
		break;
		case "PL_Q8_13":
			dialog.text = "Сначала мы возьмём Порто Белло, чтобы не оставлять гарнизон испанцев в тылу. Затем двинемся на Панаму через джунгли. Кстати, Порто Белло атаковать раньше времени бесполезно, поэтому можешь даже и не пытаться.";
			link.l1 = "Да я и не собирал"+ GetSexPhrase("ся","ась") +"!";
			link.l1.go = "PL_Q8_14";
		break;
		case "PL_Q8_14":
			dialog.text = "Хе-хе, знаю я "+ GetSexPhrase("таких шустрых малых, сам такой.","тебя, шуструю девчонку!") +" Главное: от тебя, кроме успешных действий в бою, потребуется ещё решить кое-какие вопросы. Расскажу, когда придёт время.";
		...
		case "PL_Q8_ready_1":
            ...
            {
				dialog.text = "Отлично! Итак, мы выступаем. В качестве капитанов в деле ещё участвуют Джекмен, Джон Моррис и наш большой друг Ричард Соукинс. На тебя я возлагаю командование эскадрой и штурм Порто Белло.";
				link.l1 = "Спасибо за доверие, адмирал. Я не подведу, будь уверен!";
				link.l1.go = "PL_Q8_ready_2";
			}
			else
			{
				dialog.text = "Послушай, " + pchar.name + ", я же тебе английским языком сказал, чтобы ты приходил"+ GetSexPhrase("","а") +" с одним кораблём. Тебе нужно делать лишь то, что я говорю. Что тут непонятного?!";
		...
		case "PL_Q8_timeOver_1":
			dialog.text = "Пусть у тебя 'так получается' где-нибудь в другом месте. Надеюсь, тебе быстро снесут голову испанцы с таким подходом к работе. В общем, мы уходим, как-нибудь обойдёмся без тебя в нашем деле. Прощай.";
		...
		case "PL_Q8_PBResidence_3":
			dialog.text = "М-да, неважные дела. Сдаётся мне, что это опять происки Соукинса. Теперь испанцы наверняка готовы к нашему приходу и усилят оборону Панамы.";
		...
		case "PL_Q8_PBResidence_7":
			...
			{
				dialog.text = "Ты высадил"+ GetSexPhrase("ся","ась") +" в Дарьенском заливе, и это правильно. Следуй туда, собери своих людей и двигай к Панаме. Я буду идти к цели со своими бойцами отсюда, из Порто Белло. У ворот Панамы встретимся и разом начнём штурм с двух направлений.";
				link.l1 = "Я понял"+ GetSexPhrase("","а") +", адмирал!";
				link.l1.go = "PL_Q8_PBResidence_8";
			}
			else
			{
				dialog.text = "Отправляйся в Дарьенский залив. Собери своих людей и двигай к Панаме. Я буду идти к цели со своими бойцами отсюда, из Порто Белло. У ворот Панамы встретимся и разом начнём штурм с двух направлений.";
				link.l1 = "Я понял"+ GetSexPhrase("","а") +", адмирал!";
				link.l1.go = "PL_Q8_PBResidence_8";
			}
		break;
		...
		case "PL_Q8_PBResidence_over":
			dialog.text = "Не задерживайся, вперёд к Панаме!";
		...
		case "PL_Q8_ExTPanama":
			...
			if(PChar.QuestTemp.piratesLine == "Panama_RichardBadWork")
			{
				if(LAi_IsDead(&Characters[iTemp]))
				{
					...
					link.l1 = "Да, но ведь сейчас он мёртв! А значит, дело сделано - Ричарду можно готовить надгробный камень.";
					link.l1.go = "PL_Q8_ExTPanama_1";
				}
				else
				{
					dialog.text = "Итак, ты не смог"+ GetSexPhrase("","ла") +" выполнить задание! Наш друг разгуливает здесь как ни в чём ни бывало.";
		...
		case "PL_Q8_ExTPanama_1":
			...
			link.l1 = "До этого боя у городских стен Панамы нам пришлось выдержать ещё три атаки. Даже индейцы нас атаковали!";
		...
		case "PL_Q8_ExTPanama_7_Soukins":
			dialog.text = "Ну ещё бы!";
			link.l1 = "Раз такой умный, взял бы и сам его застрелил.";
			link.l1.go = "PL_Q8_ExTPanama_8_Soukins";
		break;
		case "PL_Q8_ExTPanama_8_Soukins":
			dialog.text = "Это я понял! Итак, на этом и остановимся. Ты можешь возвращаться к своему кораблю, в Панаму тебе хода нет. За то, что ты подвёл"+ GetSexPhrase("","а") +" меня, исключаю тебя из Берегового Братства. Пош"+ GetSexPhrase("ел","ла") +" вон!";
		...
		case "PL_Q8_Panama1_2":
			...
			link.l1 = "У коменданта. Мне стоит как следует осмотреть поле последнего боя, может, найду там коменданта. Если нет, то придётся искать в форте.";
		...
		case "PL_Q8_Panama2_3":
			dialog.text = "Наши бравые пираты много чего припрятали у себя в карманах. Сегодня я заставлю сдать все в общий котёл, а потом поделим всю добычу по законам Берегового Братства.";
		...
		case "PL_Q8_Panama2_4":
			dialog.text = "Ты свою добычу уже сдал"+ GetSexPhrase("","а") +", так что я претензий к тебе не имею - можешь быть свобод"+ GetSexPhrase("ен","на") +" от планируемого мной мероприятия. Но для тебя на это время есть работа\n"+
				"Отправляйся опять в форт Панамы. Там за фортом, с внешней стороны стены есть сундук. В нем должно быть ещё золото Эскориала! Это рассказал покойный губернатор Панамы.";
			link.l1 = "Чёрт, не может быть! С внешней стороны?!";
		...
		case "PL_Q8_afterPanama_5":
			dialog.text = "Да. Я оказал огромную услугу английской короне - здорово сократил количество пиратов в Карибском море, да ещё и принёс в английскую казну такие деньги!";
		...
		case "PL_Q8_afterPanama_7":
			dialog.text = "Ты ничего не понял"+ GetSexPhrase("","а") +"! Я заплатил и за себя, и за тебя, и за всех пиратов, кто хочет жить обычной жизнью! Берегового Братства больше нет, значит, нет и его адмирала. Я привёз прощение английского престола всем пиратам! А тех, кто не захочет подчиниться - буду карать.";
		...
		case "Andre_Abel_Quest_Morgan_Dialog_5":
			dialog.text = "Вот видишь, капитан " + GetFullName(pchar) + ", до чего доводят человека жадность и нежелание делиться?..\n"+
                          "Что ж, я готов написать о нём Джекмену, но при условии, что ты выполнишь одно моё поручение... Услуга за услугу, так сказать.";
		...
		case "Andre_Abel_Quest_Morgan_Dialog_17":
			dialog.text = "В тюрьму?! Голландцы кровожаднее, чем я думал - посадить посыльного в тюрьму... Ну ладно... Главное - ты доставил"+ GetSexPhrase("","а") +" письмо, а это значит, что ты выполнил"+ GetSexPhrase("","а") +" моё поручение.";
			link.l1 = "И?...";
			link.l1.go = "Andre_Abel_Quest_Morgan_Dialog_14";
		break;
