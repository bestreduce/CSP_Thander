...
//>>>>>>>>>>>>========= Разброс диалогов по персонажам =====================
            ...
            {
    			dialog.text = "Хей, капитан, ты, кажется, собрал"+ GetSexPhrase("ся","ась") +" выходить в море?";
            ...
            {
    			dialog.text = "Вы что-нибудь узнали о моём муже, "+ GetSexPhrase("сэр","мисс") +"?";
            ...
            {
                dialog.text = NPCStringReactionRepeat(""+ GetSexPhrase("Сэр","Мисс") +", я никогда не забуду, что вы для нас сделали. Вы спасли жизнь моему мужу и избавили нашу семью от долгов. Мы назовём нашего ребёнка в вашу честь.",
                         "Это опять вы, "+ GetSexPhrase("сэр","мисс") +". Я так рада вас видеть!", ""+ GetSexPhrase("Сэр","Мисс") +", мы видимся с вами уже в который раз сегодня. Это просто прекрасно!",
                         ""+ GetSexPhrase("Сэр","Мисс") +", сколько бы вы ни обращались ко мне, знайте, что я вам всегда рада.", "block", 1, npchar, Dialog.CurrentNode);
    			link.l1 = HeroStringReactionRepeat("Ну что вы, пустое... Но знаете, всё равно приятно...",
                                               "Хорошо...",
                                               "Я тоже так думаю...",
            ...
            if (pchar.questTemp.State == "MorrisWillams_AfterFight" && npchar.id == "Jillians_Husband")   // базар в пещере с мужем, англ.линейка, квест №6
            {
    			...
    			link.l1 = "Благодарности потом. Тебя зовут Том Эттребери?";
...
//*************************************** Диалог боцмана Морриса Уильямса *********************************
 		case "Step_E6_1":
			dialog.text = "Я боцман с фрегата 'Фаворит'. Совсем недавно я видел тебя у нас на палубе, ты говорил"+ GetSexPhrase("","а") +" с кэпом...";
			link.l1 = "Верно, продолжай.";
			link.l1.go = "Step_E6_2";
		break;

 		case "Step_E6_2":
			dialog.text = "Наш кэп в таверне, он уже выпил целый бочонок рома. Он послал меня найти тебя. Он ждёт, это срочно.";
		...
		break;
 		...
 		case "Step_E6_4":
			dialog.text = "Хорошего мало, вернее, вообще ничего. Наш кэп Уильямс только что был арестован на выходе из таверны.\n"+
                          "Человек десять солдат окружили его, начальник тюрьмы показал ему какую-то бумагу, и кэпа увели.";
		...
		break;
 		...
 		case "Step_E6_5":
			...
			link.l1 = "Я иду к начальнику тюрьмы, узнаю что к чему. Не дрейфь, иди на свою посудину и следи за порядком. Вытащу я твоего капитана, можешь не сомневаться.";
...
//*********************************** Диалоги Джиллиан Эттербери *************************************
		case "Step_E6_25":
			dialog.text = "В чем дело, "+ GetSexPhrase("сэр","мисс") +"?";
			link.l1 = "Не пугайтесь, я не причиню вам зла. Мне всего лишь нужно свести счёты с ростовщиком. Насколько я понимаю, у нас общие интересы.";
			link.l1.go = "Step_E6_26";
		break;

        case "Step_E6_26":
			dialog.text = "(плача) "+ GetSexPhrase("Сэр","Мисс") +", это ужасно. Мой муж Том пропал.";
			link.l1 = "Успокойтесь и расскажите подробней, пожалуйста.";
			link.l1.go = "Step_E6_27";
		break;

        case "Step_E6_27":
			dialog.text = "Мы задолжали ростовщику деньги и не выплатили долг вовремя. Тогда он пришёл к нам домой и заявил мужу, что бросит его в тюрьму, если в течение недели мы не вернём долг с процентами.\n"+
                     "Мы отдали все, что у нас было, но это не покрыло процентов, набежавших за год на сумму долга.\n"+
                     "Муж просил его об отсрочке платежа, но получил лишь ещё неделю времени на возврат оставшейся суммы.\n"+
                     "Тогда муж пошёл искать деньги. Денег он не раздобыл, но узнал нечто такое, что, по его словам, позволит нам не бояться оказаться в тюрьме.\n"+
                     "Вчера он весь день пропадал в городе, потом заскочил на минуту, собрался и ушёл в ночь. Он до сих пор не вернулся. Я очень боюсь!";
			link.l1 = "А куда он пошёл? Он хоть что-нибудь сказал?";
			link.l1.go = "Step_E6_28";
		break;

        case "Step_E6_28":
			dialog.text = "Он сказал только, что идёт за город. Больше ничего, он очень спешил (плачет). "+ GetSexPhrase("Сэр","Мисс") +", я умоляю вас, помогите мне найти моего мужа!";
...
//**************************** Диалоги бандита в пещере, поиски мужа Джиллиан ****************************
 		case "Step_E6_30":
			dialog.text = "Кончайте "+ GetSexPhrase("его","её") +", нам свидетели ни к чему...";
			link.l1 = "Хм...";
...
//************************************** Диалоги мужа Джиллиан ***********************************
		...
		break;
 		...
 		case "Step_E6_33":
			...
			link.l1 = "Все это мне известно, рассказывай с того момента, как ты ушёл за город ночью. Что ты раскопал о ростовщике?";
			link.l1.go = "Step_E6_34";
		break;

 		case "Step_E6_34":
			dialog.text = "Этот дьявол оказался связан с контрабандистами! Вчера я заметил, что в его дом с заднего хода зашёл странный незнакомец. Я подождал, когда он выйдет, и отправился за ним.\n"+
                          "Мне удалось проследить за ним практически до этого места, когда я попал в засаду. Меня схватили и приволокли сюда. Они хотели убить меня, но решили сначала выяснить, кто я, и возможно, заработать на моём пленении.\n"+
                          "Вы поспели вовремя, "+ GetSexPhrase("сэр","мисс") +". Благодарю вас от всего сердца!";
...
//************************************** Диалоги спасшегося буканьера ***********************************
  		case "Step_E8_1":
			dialog.text = "Испанцы на трёх галеонах подошли к бухте, высадились и взяли наше поселение штурмом.";
			link.l1 = "Чёрт возьми! А где Мэнсфилд?";
			link.l1.go = "Step_E8_2";
		break;

   		case "Step_E8_2":
			dialog.text = "Убит, как и многие другие. Около половины буканьеров успели отступить из поселения. И хорошо, что они ушли, иначе конец пришёл бы всем.";
		...
		break;
   		...
   		case "Step_E8_3":
			dialog.text = "Их было очень много и вооружены они были лучше некуда.\n"+
                          "Они сняли с кораблей кулеврины и просто расстреливали нас, тогда как мы ружейным огнём не могли их достать.\n"+
			...
			link.l1 = "Чёрт возьми! Я приехал"+ GetSexPhrase("","а") +" к Мэнсфилду по поручению Мэдифорда. Я долж"+ GetSexPhrase("ен","на") +" забрать приготовленное для него золото.";
			link.l1.go = "Step_E8_4";
		break;

   		case "Step_E8_4":
			dialog.text = "Все забрали испанцы. Мэнсфилд мертв.";
			link.l1 = "Что ещё ты можешь сказать?";
		...
		break;
   		...
   		case "Step_E8_6":
			dialog.text = "Когда сюда ворвались испанцы, я притворился мёртвым и лежал вон там, в стороне.\n"+
