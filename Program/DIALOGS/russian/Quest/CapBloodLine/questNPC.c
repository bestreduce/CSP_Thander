...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
        		...
        		switch (rand(5))
					{
						...
						case 3:
							dialog.text = "Какая погода, а я вынужден стоять здесь. Моё единственное развлечение заключается в отгонянии мух от своего лица.";
    		...
    		{
                dialog.text = RandPhraseSimple("С каких это пор рабы разгуливают, где попало, с оружием?", "Ты же раб полковника Бишопа, почему ты вооружён?");
    		...
    		if (npchar.id == "Winterwood")
    		{
                ...
                {
                    dialog.text = "Какого чёрта тебе здесь надо?! Убирайся немедленно!";
    		...
    		if (npchar.id == "Weston")
    		{
				dialog.text = "Пошёл прочь из моего дома!";
				link.l1 = "Уже ухожу.";
				link.l1.go = "exit";
    		}
    		...
    		if (npchar.id == "MoneySpy")
    		{
                ...
                else
                {
                    ...
                    dialog.text = "Наконец-то! Я уже думал, ты не придёшь! Так, кх-м, тебе удалось узнать что-нибудь?";
		...
		case "Citizen_0":
            ...
            if (!CheckAttribute(npchar, "quest.btmp"))
            {
                ...
                if(rand(3)==2)
                {
                    dialog.text = LinkRandPhrase("Рысью в таверну пробежал - там ищи. Не забудь посмотреть под столами.", "Недавно зашёл в таверну, вроде. Если правильно помню, не выходил.", "Заскочил в кабак только что... с похмелья, что ли?");
                    link.l1 = "Спасибо.";
                    link.l1.go = "Exit";
                }
                else
		...
		case "Merchant_VolverstonRum":
			...
			link.l2 = "Что же так дорого то? Я ещё вернусь.";
    		link.l2.go = "exit";
		...
		case "Merchant_VolverstonRum_2":
        	dialog.text = "Ладно. Плати ещё тысячу, или проваливай.";
			if (pchar.money >= 1000)
			{
		...
		case "Merchant_VolverstonRum_3":
        	...
        	dialog.text = "Ха, да у тебя, небось, ещё деньги остались? Ладно, забирай свою бутылку. И не надоедай мне больше, раб.";
		...
		case "SoldierNotBlade":
            ...
            {
                dialog.text = LinkRandPhrase("Какого чёрта ты бегаешь по городу с обнажённым клинком? Немедленно убери оружие!", "Приказываю вам немедленно убрать оружие в ножны!", "Эй, приятель, прекрати пугать народ! Убери оружие в ножны.");
        ...
        case "SLQStep_1":
            ...
            dialog.text = "Доктор, в моём положение следовать вашему совету затруднительно. Денег у меня нет, но есть кое-что, что может сильно вам пригодиться. Так, что если вам всё-таки удастся раздобыть что-нибудь для меня, то я в долгу не останусь.";
        ...
        case "SLQStep_2":
            ...
            if (GetCharacterItem(pchar,"migraine_potion") > 0)
            {
            	link.l1 = "Да, я принёс лекарство от мигрени.";
        ...
        case "WWStep_0":
            ...
            dialog.text = "Что ты несёшь, рабская мразь?! Какое у тебя может быть ко мне дело?!";
    ...
    case "WWStep_7":
        ...
        dialog.text = "Что ты несёшь, рабская мразь?!";
	...
	case "QHStep_2":
		...
		dialog.text = LinkRandPhrase("Ты настоящий друг! Давай за знакомство!",
									 ...
									 "За твоё здоровье, и за твоё скорейшее освобождение!");
		link.l1 = "Скажи мне, братец, какие байки ходят по тавернам?";
		link.l1.go = "QHStep_3";
	break;

	case "QHStep_3":
		dialog.text = "Да ничего особенно нового... приехал недавно какой-то купец рейсом с Ямайки. Поговаривают, привёз очень ценный груз. Если ничего не путаю, он вроде старый знакомый полковника Бишопа. Разместился в одном из домов города, собирается пробыть тут ещё день-два.";
    ...
    case "MSStep_2":
        ...
        dialog.text = "Все? О чем ты?! Пошёл прочь, треклятый авантюрист!";
    	link.l1 = "...";
        link.l1.go = "exit";
    ...
    case "MSStep_3":
    	...
    	link.l1 = "У них лёгкое оружие, они не выставляют его напоказ.";
        link.l1.go = "MSStep_5";
    ...
    case "MSStep_7":
        ...
        dialog.text = "Гвардейцев? (смеётся) Клоун, хоть и сообразительный! Иди к дьяволу!";
    	link.l1 = "...";
        link.l1.go = "exit";
    ...
    case "MSStep_8":
        ...
        dialog.text = "Старый пёс не выдаст меня. Как они попали в город?";
    ...
    case "MSStep_9":
        ...
        dialog.text = "На... корабле? Но... ты лжёшь! Провались ты! ";
    	link.l1 = "...";
        link.l1.go = "exit";
    ...
    case "FStep_2":
        ...
        dialog.snd = "Voice\HADI\HADI028";
        dialog.text = "Ты дьявольски прав, чёрт возьми! За это и выпьем!";
    ...
    case "FStep_4":
    	...
    	link.l1 = "КАКОГО раньше не было? Ладно, выпьем ещё за ТАКОЕ!";
        link.l1.go = "FStep_6";
    ...
    case "FStep_5":
        ...
        dialog.text = "Нет? Нет рыбы?! Надо знать места, недотёпа! Я нашёл уникальное место... сеть наполняется до отказа за ночь - Господь свидетель!";
    	link.l1 = "Места... я тоже искал места, везде ловил... нет тут рыбы, и мест нет. К чёрту рыбную ловлю - бесприбыльное это дело!";
        link.l1.go = "FStep_7";
    ...
    case "FStep_Fail":
        ...
        dialog.text = "Ах ты... пошёл прочь, рабский лазутчик!..";
    ...
    case "QSTStep_0":
        ...
        dialog.text = "Кто ты такой вообще? Моё имя Алекс Уиннер, я торговец! Убирайся, а то перекуплю тебя у твоего хозяина и отправлю на дно в обнимку с ядром!";
    	link.l1 = "Эм...";
        link.l1.go = "Exit_Away";
        Pchar.questTemp.sLocator = "reload1_back";
    ...
    case "QSTStep_4":
        ...
        dialog.text = "Что?!";
    	link.l1 = "Ты предал меня, Гиннес! Из-за тебя я лишился всего!.. Ты умрёшь!";
    ...
    case "QSTStep_6":
        ...
        dialog.text = "Ты... ладно. Вот, возьми, забирай, чёрт! ";
    	link.l1 = "Премного благодарен, дорогой друг.";
        link.l1.go = "Exit_Away";
        Pchar.questTemp.sLocator = "reload1_back";
        if(npchar.location == "CommonRoom_MH7") Pchar.questTemp.sLocator = "reload1"
    ...
    case "JSTStep_9":
        ...
        dialog.text = "Опуская подробности - найди мне того, кто согласится доставить меня в Порто-Белло за три тысячи, и я дам тебе полторы. По рукам? Думаю, в твоём положении ни один песо лишним не будет?";
    ...
    case "JSTStep_10":
        ...
        dialog.text = "Одно условие, мон шерри - никаких имён. Ну, то есть - своё называй, если хочешь... а вот моё... скажи - я его забыл... Или... лучше скажи, что я красивая юная леди!";
    ...
    case "MNStep_0":
    	...
    	link.l1 = "Добрый день, моё имя Питер Блад. Могу я просить вас об одолжении, капитан? Одному моему, гм... другу очень нужно добраться до Порто Белло, и он готов распрощаться с тремя тысячами пиастров.";
        link.l1.go = "MNStep_1";
    ...
    case "MNStep_3":
    	...
    	link.l1 = "Честность - клинок обоюдоострый. Всего наилучшего. Надеюсь, вы не пожалеете о своём решении.";
    ...
    case "ASStep_0":
    	...
    	link.l1 = "Я Питер Блад, э-э-э... местный врач. Вы будете ждать, пока я предъявлю вам свои документы? И что здесь, чёрт возьми, происходит?";
        link.l1.go = "ASStep_1";
    ...
    case "ASStep_1":
        ...
        dialog.text = "Испанцы... они... Тот большой корабль, которому сегодня разрешили так спокойно войти под чужим флагом в нашу бухту, оказался испанским капером. Его хитрость с флагом осталась нераскрытой настолько, что, не возбудив подозрений, он преспокойно вошёл в бухту и отсалютовал форту в упор бортовым залпом из двадцати пушек...";
