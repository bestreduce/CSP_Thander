...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
            ...
            {
    			link.l1 = "Месье, я хотел"+ GetSexPhrase("","а") +" бы поговорить с Генри Морганом. Где я могу его застать?";
            ...
            {
    			link.l2 = "Послушай, ты дела Моргана ведёшь. Не встречались ли тебе в призовых бухгалтерских отчётах различные индейские предметы?";
    			link.l2.go = "Step_As_1";
            }
            if (pchar.questTemp.Ascold == "SeekThreeObject" && CheckAttribute(pchar, "questTemp.Ascold.Oliver"))
            {
    			link.l2 = "Оливер, может, ты передумаешь насчёт цены тех индейских предметов, о которых я тебе говорил"+ GetSexPhrase("","а") +"?";
            ...
            if (pchar.questTemp.Azzy == "DestrContract" || pchar.questTemp.Azzy == "2DestrContract")
            {
                ...
                {
        			link.l2 = "Оливер, послушай, хочу у тебя попросить помощи. Мне позарез нужны индейские побрякушки. Речь идёт о трёх хрустальных черепах тольтеков и странной вещи, похожей на погремушку.";
		...
		//********************* Голландская линейка *********************
        case "Step_H3_1":
            dialog.text = "А кто ты так"+ GetSexPhrase("ой","ая") +" и зачем тебе понадобился Генри?";
            link.l1 = "Меня зовут " + GetFullName(pchar) + ", к Моргану меня отправил Эдвард Мэнсфилд.";
            link.l1.go = "Step_H3_2";
        break;
        case "Step_H3_2":
            dialog.text = "Голланд"+ GetSexPhrase("ец","ка") +"?... Ты - голланд"+ GetSexPhrase("ец","ка") +"?";
            link.l1 = "Какое тебе дело, какой я нации? У меня дело к Моргану от Мэнсфилда, чего тебе ещё не ясно?";
            link.l1.go = "Step_H3_3";
        break;
        case "Step_H3_3":
            dialog.text = "Не кипятись, "+ GetSexPhrase("минхер","госпожа") +", все нормально. Голландцев среди пиратов много, а вот в Порт Рояле тебе сейчас находиться небезопасно.\n"+
                          "Поэтому вот что я тебе скажу: Морган на Антигуа, в своём доме. Там тоже будь аккуратнее - если я в тебе признал голланд"+ GetSexPhrase("ца","ку") +", то это не будет тайной и для других.\n"+
                          "И ещё - в дом к Моргану так просто не попасть, дверь работает только на выход, хе-хе... Ищи подземелье под Сент-Джонсом, оно приведёт тебя к Генри...";
		...
		//********************* Французская линейка *********************
 	 	case "Step_F8_1":
			dialog.text = NPCStringReactionRepeat("Кто вы так"+ GetSexPhrase("ой, сэр","ая, мисс") +"?", "Я уже ответил вам...",
        ...
        break;
        case "Step_F8_3":
            dialog.text = "Ладно, ладно, "+ GetSexPhrase("месье","мадемуазель") +"... Морган сейчас находится в своём доме на Антигуа.";
            link.l1 = "Ну вот и славно. Немедленно отправляюсь туда!";
            link.l1.go = "Step_F8_4";
        break;
        case "Step_F8_4":
            dialog.text = ""+ GetSexPhrase("Месье","Мадемуазель") +", имейте в виду, что в дом к Моргану в Сент-Джонсе так просто не попасть, дверь там всегда на замке. Вход - через подземелье...";
            link.l1 = "Разберемся...";
            link.l1.go = "exit";
        break;
		//********************* Квест Асокльда *********************
   		case "Step_As_1":
			...
			link.l1 = "Ты не кипятись. Всё дело в том, что я ищу три предмета индейского культа. Для покупки. Все одно - вы либо выбрасываете эту дрянь, либо продаёте за бесценок. Я готов"+ GetSexPhrase("","а") +" заплатить щедро за эти вещи, если они у вас есть.";
		...
		break;
   		case "Step_As_3":
			...
			link.l1 = "Я понимаю. Я готов"+ GetSexPhrase("","а") +" заплатить две цены за эти индейские побрякушки, только в непереплавленном виде, так сказать... А вообще, эти вещи у тебя есть? Или мы бестолковый разговор ведём?";
			link.l1.go = "Step_As_4";
		break;
   		case "Step_As_4":
			dialog.text = "Эти вещи у меня есть. И, в принципе, я готов буду тебе их продать. Но только две цены - не предмет для серьёзного разговора. Слишком мало.";
		...
		break;
   		case "Step_As_7":
			dialog.text = "Ладно, хватит болтать. Берёшь или нет?";
            ...
            {
    			link.l2 = "Я бы взял"+ GetSexPhrase("","а") +", да таких денег нет. Пойду, может заработаю ещё...";
    			link.l2.go = "exit";
            }
		break;
   		case "Step_As_8":
		...
		case "PL_Q8_over":
			...
			link.l1 = "Слушай, Оливер, мне нужнен Морган!";
			link.l1.go = "PL_Q8_over_1";
		break;
		case "PL_Q8_over_1":
			dialog.text = "А его нет, он ушёл на Панаму без тебя. А тебе велено передать, что ты можешь загорать на пляжах.";
			link.l1 = "Чего?!";
			link.l1.go = "PL_Q8_over_2";
		break;
		case "PL_Q8_over_2":
			dialog.text = "Того! Перевожу специально для тебя. Ты не успел"+ GetSexPhrase("","а") +" к сроку, и теперь ты не нуж"+ GetSexPhrase("ен","на") +". Прости...";
