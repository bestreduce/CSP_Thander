...
//>>>>>>>>>>>>========= Разброс диалогов по персонажам =====================
            ...
            {
    			dialog.text = "Эй, "+ GetSexPhrase("приятель","подруга") +"! Нам нужно потолковать с твоим дружком. Отойди-ка в сторонку и не мешай - здоровее будешь...";
            ...
            {
    			dialog.text = "Сеньор"+ GetSexPhrase("","ита") +", кто вы? И что вам нужно в моём доме?";
			...
			if (npchar.id == "InesDeLasCierras" && pchar.questTemp.State == "Fr5AnnaHant_GoodWork")
            {
    			...
    			link.l1 = "Всё в порядке, проблем с родственниками донны Анны более не существует...";
    			link.l1.go = "Step_F5_4";
            }
			if (npchar.id == "Captain_1" || npchar.id == "Captain_2")//квест №6, базар с кэпами
            {
    			dialog.text = ""+ GetSexPhrase("Месье","Мадемуазель") +", нам нужно на всех парусах идти к Кумане. Не приведи Гоcподь, опоздаем - и там не будет реалов! Вы не знаете нашего капитана...";
			...
			if (npchar.id == "RockBrasilian" && pchar.questTemp.State == "Fr7RockBras_toSeekPlace")//квест №7, базар с Бразильцем.
            {
				...
				{
    				dialog.text = "Кто ты?..";
    				link.l1 = "Я приш"+ GetSexPhrase("ел","ла") +" за тобой, меня послал д'Ожерон...";
			...
			if (npchar.id == "RockBrasilian" && pchar.questTemp.State == "empty")//квест №7, базар с Бразильцем в резиденции дОжерона.
            {
    			...
    			link.l1 = "Всё в порядке, капитан Рок...";
...
//********************************* Арест ГГ у Стэвезанта. Квест №2 **********************************
		...
		break;
 		case "Step_F2_2":
			...
			link.l1 = "О том, что на мою казнь ты посмотришь задаром. Зато станешь на несколько кругляшек побогаче, если до завтра Стэвезант обыщет капитанскую каюту на моём корабле.";
		...
		break;
 		case "Step_F2_5":
			...
			link.l1 = "Так я же и не спорил"+ GetSexPhrase("","а") +"... Ладно, послушай, не теряй времени, приступай к делу. А то меня точно повесят ни за что.";
		...
		break;
 		case "Step_F2_8":
			...
			link.l1 = "Очень рад"+ GetSexPhrase("","а") +". Глупо было бы умереть ни за что.";
			link.l1.go = "Step_F2_9";
		break;
 		case "Step_F2_9":
			dialog.text = "Вам вернут все, что было у вас конфисковано. Передавайте мои наилучшие пожелания Бертрану. И будьте осторожны на обратном пути - в последнее время близ острова творится чёрт знает что.";
		...
		break;
 		...
 		case "Step_F2_12":
			dialog.text = "Никогда я, " + GetFullName(npchar) + ", испанский капер, не пасовал перед ладроном! Я потопил в окрестностях Кюрасао двадцать кораблей, так что тебе меня не запугать. Лучше принять смерть, но сохранить честь!";
			link.l1 = "Ну что же, прими смерть, раз так желаешь. Я узнал"+ GetSexPhrase("","а") +" все, что нужно...";
...
//********************************* Похищение донны Анны. Квест №3 **********************************
 		case "Step_F3_1":
			dialog.text = "Хосе мертв... Видит Господь, я этого не хотела... Он нашёл письмо, которое я писала ему, и запер меня здесь...";
...
//********************************* Проблемы донны Анны. Квест №5 **********************************
 		case "Step_F5_1":
			dialog.text = "Да, это так. Сеньор"+ GetSexPhrase("","ита") +", донне де Лейва грозит смертельная опасность! Генерал-губернатор Гаваны Франсиско Орегон-и-Гаскон предпринял расследование по этому поводу и выяснил все обстоятельства бегства донны Анны на Тортугу.\n"+
		...
		break;
 		case "Step_F5_3":
			...
			link.l1 = "Надо успеть перехватить их в джунглях! Если они успеют вернуться в город, мне придётся воевать со всем испанским гарнизоном. Спасибо вам, уважаемая Инес. Я никогда не верил"+ GetSexPhrase("","а") +" в женскую дружбу, но вы, отважная женщина, убедили меня в обратном. Прощайте.";
    		link.l1.go = "exit";
...
//********************************* Вызволение Рока Бразильца. Квест №7 **********************************
 		case "Step_F7_1":
    		if (LAi_group_GetTarget(pchar) <= 0)
			{
				...
				link.l1 =  "Не кто иной, как месье Бертран д'Ожерон. Полагаю, ты знаешь, кто это... Ну ладно, теперь давай убираться отсюда на корабль, нас ждут на Тортуге. Ты можешь держать оружие?";
		...
		break;
 		case "Step_F7_4":
			...
			AddQuestUserData("Fra_Line_7_RockBras", "sSex", GetSexPhrase("ел","ла"));
