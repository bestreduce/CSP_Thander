	...
	switch(Dialog.CurrentNode)
	{
		...
		case "PiratesMan1":
			if (pchar.questTemp.State == "" || sti(pchar.questTemp.CurQuestNumber) > 8 || sti(pchar.questTemp.NationQuest) == SPAIN)
			{
				...
				{
					dialog.text = "Здорово, "+ GetSexPhrase("парень","деваха") +"! Я смотрю, кораблик у тебя имеется... Не хочешь ли поработать на Береговое Братство?";
					link.l1 = "Извини, нет. Не хочу переходить дорогу могучим державам.";
					link.l1.go = "exit_hl";
		...
		case "ATpr_F3":
			...
			link.l2 = "Извини, нет. Не хочу переходить дорогу могучим державам.";
		...
		break;
		case "Tavern_2":
			dialog.text = "Да, все просто. Берёшь к себе на борт одного человечка, Морган его уже заждался, и – в Порт-Рояль. А там уж Морган вас встретит... и расплатится там же. Как видишь, ничего сложного. И не забывай, этим ты окажешь Моргану большую услугу. Ну как, соглас"+ GetSexPhrase("ен","на") +"?";
