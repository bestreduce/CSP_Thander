	...
	switch(Dialog.CurrentNode)
	{
		...
		case "PiratesMan1":
			if (pchar.questTemp.State == "" || sti(pchar.questTemp.CurQuestNumber) > 8 || sti(pchar.questTemp.NationQuest) == SPAIN)
			{
				...
				{
					dialog.text = "Здорово, "+ GetSexPhrase("парень","деваха") +"! Что ты на меня так вылупил"+GetSexPhrase("ся","ась")+".";
					link.l1 = "Да так, мимо проходил"+GetSexPhrase("","а")+".";
					link.l1.go = "exit_hl";
					link.l2 = "Это не ты ли случаем капитан Гудли?";
			...
			else
			{
				...
				link.l1 = "А в чём дело-то? Я чем-то запятнал"+ GetSexPhrase("","а") +" себя в глазах Берегового Братства?";
		...
		case "ATpr_F1":
			...
			link.l1 = "Анжелика Тич, к твоим услугам.";
			link.l1.go = "ATpr_F2";
		break;
		case "ATpr_F2":
			dialog.text = "Быть не может! Та самая малышка Ангелика? Ты верно меня не помнишь, я Гудли. Я служил у Эдварда Тича, не долго правда, был молод и амбициозен, довольно быстро обзавёлся собственной лоханкой.";
			link.l1 = "Капитан Гудли? Так я и тебя и ищу. Меня послал Джекман, так как тебе нужна какая-то помощь.";
			link.l1.go = "ATpr_F3";
		break;
		case "ATpr_F3":
			dialog.text = "Нужно оказать одну услугу Генри Моргану. Заглядывай в таверну, там всё обсудим.";
			link.l1 = "Моргану, говоришь? Хм, ну хорошо.";
			link.l1.go = "PiratesMan1_2";
			link.l2 = "Я щас занята, давай позже.";
		...
		//<== Sinistra Пролог Анжелика тич
		case "PiratesMan1_1":
			dialog.text = "Да, это я. Что ты хотел"+GetSexPhrase("","а")+"?";
			link.l1 = "Меня прислал к тебе Джекмен. Он сказал, что тебе требуется помощь.";
			link.l1.go = "PiratesMan1_2";
		break;
		case "PiratesMan1_2":
			dialog.text = "Нужно оказать одну услугу Генри Моргану. Приходи в таверну, там всё обсудим.";
		...
		break;
		case "Tavern_2":
			dialog.text = "Да, всё просто. Берёшь к себе на борт одного человечка, Морган его уже заждался, и – в Порт-Рояль. А там уж Морган вас встретит... и расплатится там же. Как видишь, ничего сложного. И не забывай, этим ты окажешь Моргану большую услугу. Ну как, соглас"+ GetSexPhrase("ен","на") +"?";
			link.l1 = "Хм, невелика услуга... А что за человек?";
			link.l1.go = "Tavern_3";
			link.l2 = "Морган, конечно, известный пират, но у меня есть другие дела, и я не намерен"+ GetSexPhrase("","а") +" всё бросать. И вообще, я не кэп курьерского баркаса!";
		...
		case "Tavern_again":
			dialog.text = "Джон ждёт тебя в бухте Пуэрто-Принсипе. Не тяни с этим!";
			link.l1 = "Хорошо...";
			link.l1.go = "exit";
			NextDiag.TempNode = "Tavern_again";
		break;
		...
		case "JohnBolton_inWay":
			dialog.text = "Мы ещё не добрались до Порт Рояля, а я спешу, "+ GetSexPhrase("приятель","подруга") +"...";
			link.l1 = "Мы идём туда, не волнуйтесь.";
			link.l1.go = "exit";
			NextDiag.TempNode = "JohnBolton_inWay";
		break;
		...
		case "ArrestInPR":
			...
			link.l1 = "Эй, приятель, а ты не ошибся? Я прив"+ GetSexPhrase("ёз","езла") +" этого человека по поручению Генри Моргана!";
			link.l1.go = "ArrestInPR_1";
		break;
		case "ArrestInPR_1":
			dialog.text = "Комендант разберётся, где ошибка, а где нет. Следуйте за мной!";
			link.l1 = "Вот чёрт...";
		...
		break;
		case "EdwardLoy_4":
			dialog.text = "Все! Достаточно! Ты сделал"+ GetSexPhrase("","а") +" своё подлое дело, теперь убирайся отсюда!";
		...
		case "CapGoodly_2":
			...
			link.l1 = "Сдаётся мне, друг Гудли, что ты, как в истории с Болтоном, опять чего-то недоговариваешь. Небось уже прикидываешь, сколько Морган отсыплет за голову Лоу? Так вот знай – я буду идти за тобой по пятам. А когда ты выведешь меня на Лоу, я кончу тебя первого, чтоб под ногами не путался.";
		...
		case "SeaWolf":
			dialog.text = "Дьявол! Ты всё-таки добрал"+ GetSexPhrase("ся","ась") +" до меня!";
			link.l1 = "А разве могло быть иначе?!";
			link.l1.go = "SeaWolf_1";
		break;
		case "SeaWolf_1":
			dialog.text = "Да что ты о себе возомнил"+ GetSexPhrase("","а") +"?! Думаешь, ты так"+ GetSexPhrase("ой","ая") +" всемогущ"+ GetSexPhrase("ий","ая") +"?! Ты прост"+ GetSexPhrase("ой порученец","ая наёмница") +" Моргана, не более того!";
		...
		/*case "PQ5CapGoodly_2":
			dialog.text = "Проблема в том, что Моргану я сейчас нужен в Порт-Рояле, и он не хочет меня отпускать. Выполнить заказ нет никакой возможности... Короче, возьмёшь мой заказ?";
			link.l1 = "А отказаться ты не можешь?";
			link.l1.go = "PQ5CapGoodly_3";
		break;*/
		case "PQ5CapGoodly_3":
			dialog.text = "Если б все решалось так просто, я бы тебя не просил. Решайся. Всю награду от заказчика заберёшь себе.";
		...
		break;
		case "PQ5CapGoodly_7":
			dialog.text = "Чёрт с тобой. Я доплачу своими деньгами. Просто я не могу отказать этому человеку.\n Желаю тебе удачи. Я буду ждать в городе.";
		...
		break;
		case "PQ5Aivory_1":
			dialog.text = "Ха!! И ещё говорят, что я наглец. Ну, проверим сейчас, кто ты есть...";
			link.l1 = "Проверим...";
		...
		case "PQ5CapGoodlyEnd_1":
			dialog.text = "Ха! Ты несказанно меня обрадовал"+ GetSexPhrase("","а") +"! Гора с плеч свалилась.\nИзвини, всю сумму ещё не собрал, пока только 15 тысяч, остальные заплачу чуть позже... Будь уверен"+ GetSexPhrase("","а") +", не обману, мне не нужен враг в твоём лице.";
		...
		case "PQ5CapGoodlyEnd_4":
			dialog.text = "Нет, не в курсе. Мне он отчёта не даёт. Так что, за всеми ответами к нему.";
		...
		break;
		case "JohnLids_1":
			dialog.text = "Очень хорошо. А как твоё полное имя?";
			link.l1 = GetFullName(pchar) + ".";
			link.l1.go = "JohnLids_2";
		break;
		case "JohnLids_2":
			dialog.text = "Ха!! Не может быть! А тогда кого мы заперли в этой бухте?";
			link.l1 = "Понятия не имею. Именно я и есть " + GetFullName(PChar) +  ". Да что ты крутишь всё вокруг да около? Рассказывай в чём дело. Кого вы заперли?";
		...
		case "JohnLids_5":
			...
			link.l1 = "Да уж, час от часу не легче. Но может быть, всё-таки поможешь мне людьми?";
			link.l1.go = "JohnLids_6";
		break;
		case "JohnLids_6":
			dialog.text = "Только тем, что покараулю здесь эту братию, если тебе нужно куда-то отлучиться.";
			link.l1 = "Ну ладно, придётся, видать, само"+ GetSexPhrase("му","й") +" разговаривать со сво"+ GetSexPhrase("им","ей") +" родственни"+ GetSexPhrase("чком","цей") +".";
		...
		break;
		case "JohnLids_again":
			dialog.text = "Ты ещё не покончил"+ GetSexPhrase("","а") +" со сво"+ GetSexPhrase("им","ей") +" "+ GetSexPhrase("'братом'","'сестрой'") +", а я вечно ждать тебя не буду...";
		...
		case "QuestPirateHead":
			dialog.text = "Ха, так ты и есть " + GetFullName(pchar) + "! Рад"+ GetSexPhrase("","а") +" тебя видеть, "+ GetSexPhrase("братишка","сестрёнка") +"...";
			link.l1 = "Как"+ GetSexPhrase("ой","ая") +" я тебе "+ GetSexPhrase("братишка","сестрёнка") +"?!";
		...
		break;
		case "FriendLinney_2":
			dialog.text = "А-а, ну так бы сразу и сказал"+ GetSexPhrase("","а") +". Я давно знаю Стива, и сейчас он рассказал мне о поручении Моргана. В общем, в Ла Веге он ничего не разузнал. Он должен был здесь встретиться с одним человеком, который якобы что-то знает, но человек на встречу не явился. Стив планировал уже уходить в другой город, но вечером к нему подошли два неизвестных мне человека. Они переговорили, и Стив ушёл с ними.\n"+"Понимаешь, и все бы ничего. Мало ли, может, он новых информаторов нашёл, но вчера я случайно узнал, что его шхуна 'Ласточка' продаётся на верфи Санто-Доминго. А Стив свою 'Ласточку' очень любил и сам бы ни за что не продал. Вот я и подумал, что пора уходить с Ла Веги от греха подальше. Иначе можно дождаться, что и за мной придут.";
			link.l1 = "Понятно. Может, всё-таки продал? Хотя, странно все это...";
		...
		break;
		case "LeonCapitain_5":
			dialog.text = ""+ GetSexPhrase("Идиот","Дура") +"! Ты ещё пожалеешь о своей глупости...";
		...
		case "PQ8_PirInPan":
			dialog.text = "Знаешь, " + pchar.name + ", не зря я пошёл к тебе в команду. В итоге я жив, да ещё в захваченной Панаме! Кто бы мог подумать?..";
		...
		case "PQ8_Mayor_2":
			...
			link.l1 = "Вы играете с огнём! Панаму взял Генри Морган, здесь вообще собрался весь цвет карибского пиратства. Вы представляете себе, что с вами будет?";
			link.l1.go = "PQ8_Mayor_3";
		break;
		case "PQ8_Mayor_3":
			dialog.text = "Представляю. Но я ничем не могу помочь в любом случае. Всё дело в том, что у меня нет ключа от сундука, в котором, возможно, и находится кое-что... Я говорю об одном из сундуков, находящихся в этом кабинете.";
		...
		break;
		case "PQ8_Mayor_6":
			dialog.text = "Я понимаю. Но мне остаётся надеяться только на удачу.";
			link.l1 = "Я поищу ключ. А вам я рекомендую напрячь память - возможно вы ещё что-нибудь вспомните...";
		...
		case "PQ8_FackMorgan":
			...
			link.l1 = LinkRandPhrase("Да, я слышал"+ GetSexPhrase("","а") +"...", "Чёрт!!!", "Разрази меня гром!");
			link.l1.go = "exit";
		break;
		...
		case "PL_Q8_QFackMorgan_1":
			dialog.text = "Ха!! Я тебя сейчас так обрадую, что ты от радости с ума сойдёшь!";
			link.l1 = "Говори по существу, не тяни.";
			link.l1.go = "PL_Q8_QFackMorgan_2";
		break;
		case "PL_Q8_QFackMorgan_2":
			dialog.text = "Наш адмирал, Генри Морган, собрал со всех нас добычу! Он построил пиратов и первым очистил свои карманы в общий котёл!";
			link.l1 = "Ну и что, это мне было известно. Что в этом чрезвычайного?";
			link.l1.go = "PL_Q8_QFackMorgan_3";
		break;
		case "PL_Q8_QFackMorgan_3":
			dialog.text = "В этом - ничего, но только это ещё не все новости... Мы все выложили содержимое своих кошельков и пошли заниматься своими делами в ожидании, когда Генри всё поделит по закону Берегового Братства. А в это время наш доблестный и честнейший адмирал тихо погрузился в испанский галеон и отдал концы.";
			link.l1 = "Как?.. А деньги?!";
			link.l1.go = "PL_Q8_QFackMorgan_4";
		break;
		case "PL_Q8_QFackMorgan_4":
			dialog.text = "Наши деньги тоже отдали концы вместе с Морганом! Я же тебе о чём и толкую - Генри прихватил всю добычу и бросил нас здесь, в этой чёртовой Панаме!";
		...
		case "PL_Q8_QFackMorgan_10":
			dialog.text = "Опять через джунгли? Нет уж, второго такого похода я не переживу. А с тобой никто и не пойдёт: "+ GetSexPhrase("вы же с Морганом друзья!","все уверены, что ты Моргану... это... ну, подруга.") +" А если ты заикнёшься о том, чтобы что-то собрать – уверен, что сразу получишь пулю в лоб! Теперь каждый сам за себя, капитан... Ну ладно, некогда мне тут с тобой лясы точить. Пойду в таверну, надеюсь, там ещё остался ром...";
