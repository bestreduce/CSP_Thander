...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "PiratesMan1":
			if (pchar.questTemp.State == "" || sti(pchar.questTemp.CurQuestNumber) > 8 || sti(pchar.questTemp.NationQuest) == SPAIN)
			{
				...
				else
				{
					...
					if (checkattribute(pchar, "questTemp.AnjelikaTichPrologue8_Gudli") && pchar.questTemp.AnjelikaTichPrologue8_Gudli == "ATP8")	//Sinistra Пролог Анжелика тич
					{
		...
		case "PL_Q8_QFackMorgan_3":
			dialog.text = "В этом - ничего, но только это ещё не все новости... Мы все выложили содержимое своих кошельков и пошли заниматься своими делами в ожидании, когда Генри все поделит по закону Берегового Братства. А в это время наш доблестный и честнейший адмирал тихо погрузился в испанский галеон и отдал концы.";
			link.l1 = "Как?.. А деньги?!";
			link.l1.go = "PL_Q8_QFackMorgan_4";
		break;
		case "PL_Q8_QFackMorgan_4":
			dialog.text = "Наши деньги тоже отдали концы вместе с Морганом! Я же тебе о чем и толкую - Генри прихватил всю добычу и бросил нас здесь, в этой чёртовой Панаме!";
