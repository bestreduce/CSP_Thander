...
//>>>>>>>>>>>>========= Разброс диалогов по персонажам =====================
            ...
            if (npchar.id == "SpaPrisoner1" || npchar.id == "SpaPrisoner2" || npchar.id == "SpaPrisoner3")
            {
    			...
    			{
                    dialog.text = "Сеньор"+ GetSexPhrase("","ита") +", кто вы?";
        			link.l1 = "К чёрту разговоры, сейчас я освобожу вас, и мы выбираемся отсюда. Или вам нравится английская баланда?.. Рассредоточьтесь и, не привлекая внимания, направляйтесь к моему кораблю. Ваша задача - добраться до моего корабля живыми. Через некоторое время я устрою небольшую заварушку, которая отвлечёт внимание, так что вам будет легче. Я надеюсь, вам всё понятно?";
        			link.l1.go = "Step_S1_1";
                }
                else
                {
                    dialog.text = LinkRandPhrase("Господь услышал наши молитвы!", "Если бы вы знали, как мы рады вас видеть...", "Спасибо сеньору генерал-губернатору, он не забыл о нас...");
            ...
            {
    			dialog.text = "Ты "+ GetSexPhrase("победил, проклятый кастилец","победила, проклятая испанка") +".";
            ...
            {
    			dialog.text = "Что вы хотели, "+ GetSexPhrase("месье","мадемуазель") +"? Хозяина нет дома.";
    			link.l1 = "Понятно...";
    			link.l1.go = "exit";
            }
            ...
            {
    			dialog.text = ""+ GetSexPhrase("Месье","Мадемуазель") +", вам нужно поторапливаться!";
    			link.l1 = "Да, я понял.";
    			link.l1.go = "exit";
            }
            if (npchar.id == "AnryDEstre")// квест №4, базар с самим д'Эстре
            {
    			dialog.text = "Чёрт возьми, извольте немедленно объясниться - почему вы преследуете меня?!!";
            ...
            {
    			dialog.text = "А?.. Че те надо?!";
    			link.l1 = "Да ничего особенного. Просто хотел"+ GetSexPhrase("","а") +" дать тебе несколько монет - ты ведь мучаешься, а так здоровье поправишь...";
            ...
            if (npchar.id == "Messanger" && pchar.questTemp.State == "Sp6TakeMess_waitMessanger") //квест №6, базар с голландским вестовым
            {
    			...
    			link.l1 = "Я - доверенное лицо генерал-губернатора Кюрасао. Мне необходимо переговорить с вами. Давайте пройдём в таверну, побеседуем...";
            ...
            if (npchar.id == "Messanger" && pchar.questTemp.State == "Sp6TakeMess_Action") //квест №6, базар с голландским вестовым
            {
    			...
    			link.l1 = "Кто я - неважно, а нужны мне ваши бумаги.";
    			link.l1.go = "Step_S6_13";
			}
            if (npchar.id == "OurCaptain_1") //квест №7, базар с Пардалем его на корабле до битвы
            {
    			dialog.text = "Чёрт возьми, не время для политеса! Меня атакуют ладроны, помоги мне!!!";
    			link.l1 = "Хорошо.";
    			link.l1.go = "Exit";
			}
            if (npchar.id == "OurCaptain_1" && pchar.questTemp.State == "Sp7SavePardal_GoodWork") //квест №7, базар с Пардалем его на корабле
            {
    			dialog.text = "Дьявол, ты оказал"+ GetSexPhrase("ся","ась") +" здесь как нельзя вовремя! Даже не знаю, что бы я делал без твоей помощи... Скажи мне, кому я обязан своим спасением?";
            ...
            {
    			dialog.text = "Дорогу!!!";
    			link.l1 = "Полегче, милейший! Куда это ты так несёшься?";
...
//********************************* Спасение трёх горожан. Квест №1 **********************************
 		case "Step_S1_1":
			dialog.text = "Да, сеньор"+ GetSexPhrase("","ита") +". Спасибо и да хранит вас Господь!";
...
//********************************* Расследование убийства. Квест №4 **********************************
		...
		break;
 		case "Step_S4_2":
			...
			link.l1 = "Меня зовут " + GetFullName(pchar) + ". Я приш"+ GetSexPhrase("ел","ла") +" узнать у вас, кто такая донна Анна, которую вы доставили месье д'Ожерону? ";
		...
		break;
 		case "Step_S4_4":
			...
			link.l1 = "Тогда, сударь, я вынужден"+ GetSexPhrase("","а") +" буду убить вас.";
		...
		break;
 		case "Step_S4_6":
			dialog.text = "У-у-у, да вы весьма осведомлены об этом моём приключении, даже знаете, скольких испанских дьяволов я отправил на тот свет... Я впечатлён. Так кто же вы, судар"+ GetSexPhrase("ь","ыня") +"?";
			link.l1 = "Я испанский капер, и сейчас занимаюсь расследованием убийства коменданта Гаваны и исчезновения его вдовы. Да, совсем забыл"+ GetSexPhrase("","а") +" - по поручению генерал-губернатора Гаваны дона Франсиско Орегон-и-Гаскона.";
			link.l1.go = "Step_S4_7";
		break;
 		case "Step_S4_7":
			dialog.text = "Ага, вот теперь мне все ясно, и я спокойно могу отправлять вас на тот свет. К оружию, сеньор"+ GetSexPhrase("","ита") +"!";
...
//********************************* Уничтожение Ла Веги. Квест №5 **********************************
 		case "Step_S5_1":
			dialog.text = "Агрх, "+ GetSexPhrase("испанский пёс","испанская дрянь") +"! Старый буканьер дорого продаст свою жизнь...";
...
//********************************* Перехват посыльного Сювесанта. Квест №6 **********************************
		...
		break;
 		case "Step_S6_4":
			dialog.text = "Сто монет сейчас же - и я всё сделаю. А так с места мне не сдвинуться - жуткое похмелье...";
		...
		break;
 		case "Step_S6_8":
			dialog.text = NPCStringReactionRepeat("Если вы хотите мне что-либо сказать, то говорите немедленно и здесь.",
                          "Опять ты? Я же сказал тебе, что никуда не пойду!", "Ну всё, ты меня утомил"+ GetSexPhrase("","а") +"... Стража, держи е"+ GetSexPhrase("го","её") +", это шпион"+ GetSexPhrase("","ка") +"!", "none", "block", 0, npchar, Dialog.CurrentNode);
			link.l1 = HeroStringReactionRepeat("Давайте пройдём в таверну...", "Послушайте...", "А, что?!", "none", npchar, Dialog.CurrentNode);
		...
		//=====>>> вестовой захвате в комнате таверны.
 		case "Step_S6_13":
			...
			link.l1 = "Я знаю, но если ты не отдашь мне их добровольно, то мне придётся снять их с твоего тела. Выбирай – жизнь или бумаги.";
...
//********************************* Спасение Пардаля. Квест №7 **********************************
 		case "Step_S7_1":
			...
			link.l1 = "Отлично. Ну что же, я долж"+ GetSexPhrase("ен","на") +" возвращаться в Гавану к генерал-губернатору. Нужно доложить ему об удачном завершении дела.";
...
//********************************* Спасение Пардаля. Квест №7 **********************************
		...
		break;
 		case "Step_S10_3":
			dialog.text = "Что?.. А-а-а, дайте дорогу сеньор"+ GetSexPhrase("","ита") +", мне нужно известить генерал-губернатора...";
