...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "LSCNotBlade":
			dialog.text = LinkRandPhrase("Слушай, ты бы убрал"+ GetSexPhrase("","а") +" оружие. А то нервируешь немного...", "Знаешь, у нас тут не принято сабелькой размахивать. Убери оружие.", "Слушай, что ты, как д'Артаньян, бегаешь тут, шпагой машешь? Убери оружие, не к лицу это "+ GetSexPhrase("серьёзному мужчине","девушке") +"...");
		...
		break;
		case "FS_4":
			dialog.text = "Если визитёр является "+ GetSexPhrase("хамовитым мужланом","невоспитанной девкой") +", то вполне!";
			...
			else
			{
				...
				else
				{
					...
					else
					{
						if (npchar.location == sld.location)
						{
							dialog.text = LinkRandPhrase("Простите, но она находится прямо здесь, " + npchar.quest.seekIdx.where + ". Вам нужно быть внимательн"+ GetSexPhrase("ым","ой") +".", "Хм, она ведь здесь, среди нас!", "Если бы вы посмотрели вокруг более внимательно, то наверняка уже увидели её...");
						}
						else
						{
		...
		case "ansewer_1_2":
			...
			link.l1 = "Хм, а вы весёлая дама, как я погляжу...";
		...
		break;
		case "AfterFight_2":
			dialog.text = ""+ GetSexPhrase("Молодой человек","Девушка") +", вот что я вам скажу... Я, наверное, старейший житель этого острова, и не такое ещё видела здесь.";
			link.l1 = "Вы жили здесь ещё до того, как Хилл Брюннер попал в Город?";
		...
		break;
		case "AfterFight_9":
			dialog.text = "О-о-о, это легендарный человек, его зовут Хенрик Ведекер. Абсолютное большинство граждан Города даже не знают, что он живёт с нами. С тех пор, как клан 'Каспер' обосновался на барке 'Сан Габриэль', Механика я не видела.";
			link.l1 = "Вы сказали, что его изолировали...";
			link.l1.go = "AfterFight_10";
		break;
		case "AfterFight_10":
			dialog.text = "Ну, а как это ещё можно назвать? Думаю, что адмирал держит Хенрика в изоляции специально. Всё дело в том, что Механик - необыкнованно талантливый человек в плане смастерить что-нибудь. Одно время начал он рассказывать об какой-то опасности, грозящей нам всем. Говорил о том, что мы должны покинуть Город, пока не поздно\n"+
				"Но вы знаете, чудаковатых людей воспринимают несерьёзно, вот и над его байками хохотали только. А потом вышел Закон о недопущении строительства плавательных средств, и сразу же 'касперы' заняли Сан Габриэль. Ну и о нашем чудаке как-то неприлично стало вспоминать, да и небезопасно.";
		...
		break;
		case "AfterFight_12":
			dialog.text = "Именно. Скрыть следы преступления сейчас стало очень просто. Дело в том, что под нами, на глубине, завелись какие-то жуткие твари. Если сбросить тело в воду, то оно даже не всплывёт потом...";
		...
		case "SeekCrew_2":
			...
			link.l1 = "Вы уверены в своём решении? Я не для того спасал"+ GetSexPhrase("","а") +" вас от 'касперов', чтобы вы так бездарно рисковали своей жизнью теперь.";
