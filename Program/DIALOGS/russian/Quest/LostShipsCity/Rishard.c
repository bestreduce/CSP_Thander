...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			{
				dialog.text = "О! Вот и ты, "+ GetSexPhrase("парень","девчуля") +". А я уже было решил сам тебя разыскать. Ты мне "+ GetSexPhrase("нужен дружище","нужна подруга") +"";
			...
			{
				dialog.text = "А, "+GetFullName(pchar)+"! "+TimeGreeting()+"! Ты что-то хотел"+ GetSexPhrase("","а") +"?";
				...
				link.l5 = "Да просто решил"+ GetSexPhrase("","а") +" узнать как у тебя дела. Ещё увидимся!";
				link.l5.go = "exit";
			}
		...
		case "meeting": // первая встреча
			dialog.text = "Плохо? Нет, не плохо. Просто я не в духе малость. Так чего ты хотел"+ GetSexPhrase("","а") +"?";
		...
		case "meeting_2":
			dialog.text = "Ты наверное хотел"+ GetSexPhrase("","а") +" сказать - бывший капитан, хе-хе?";
			link.l1 = "Ну да... Бывший";
			link.l1.go = "meeting_4";
		break;

		case "meeting_4":
			dialog.text = "А-а... А я-то уже подумал... Ладно, "+ GetSexPhrase("приятель","подруга") +", лучше я выпью рому...";
		...
		case "ring_2":
			dialog.text = "А что же другое о нём говорить-то, ха-ха! Ладно. Довольно предисловий, перехожу к делу. (понижая голос) Мне удалось отыскать у внешнего кольца один хорошо сохранившийся корабль. Он стоит отдельно от других среди рифов, причём ровненько-ровненько, что редкость немалая\nКорабль большой - пинас. Прибило его к нам недавно, месяца два тому или немного больше. Пока никто из других жителей его не обнаружил, так что есть шанс неплохо поживиться, покопавшись в его трюме и каютах.";
			link.l1 = "Прекрасно, я рад"+ GetSexPhrase("","а") +" за тебя. Ну а я-то тут причём? Или ты хочешь позвать меня с собой за компанию?";
			link.l1.go = "ring_3";
		break;
		...
		case "ring_4":
			dialog.text = "Ну да. Мы заключаем с тобой соглашение: я показываю дорогу на корабль, а ты - ликвидируешь крабов. Всё найденное на корабле достанется нам двоим. Так будет справедливо - я нашёл судно, ты пробил"+ GetSexPhrase("","а") +" дорогу.";
			link.l1 = "Нет, приятель, не заинтересовало меня твоё предложение. Драться в трюме полузатопленного корабля со стаей крабов ради сомнительных трофеев - это не по мне. Поищи "+ GetSexPhrase("другого рискового парня","кого-нибудь другого") +", "+ GetSexPhrase("который","кто") +" будет таскать для тебя каштаны из огня.";
			link.l1.go = "ring_exit";
			link.l2 = "Интересно... Я давно хотел"+ GetSexPhrase("","а") +" побывать на внешнем кольце. По рукам! С крабами я разберусь.";
			link.l2.go = "ring_5";
		break;
		...
		case "ring_5":
			dialog.text = "Я рад, что не ошибся в тебе, "+ GetSexPhrase("дружище","подруга") +". У меня предчувствие, что мы найдём там много чего интересного...";
		...
		case "ring_7":
			dialog.text = "Давай завтра - сначала следует подготовиться, и тебе, и мне. Запасись лечебными микстурами, пошарь в сундуках или посмотри в магазине, сойдёт и ром - в челюстях крабов содержится яд. Осторожность не помешает. Встречаемся в восемь утра на 'косой палубе'. Знаешь, где это?";
			link.l1 = "Нет. Объясни.";
			link.l1.go = "ring_8";
		break;
		...
		case "ring_10":
			dialog.text = "Ага, вот и ты, "+ GetSexPhrase("дружище","подруга") +". Готов"+ GetSexPhrase("","а") +" шинковать крабовый салат?";
			link.l1 = "Конечно!";
			link.l1.go = "ring_11";
		break;
		...
		case "ring_13":
			dialog.text = "";
			link.l1 = "Так, кажется, вижу... Да, "+ GetSexPhrase("нашёл","нашла") +"!";
			link.l1.go = "ring_13a";
		break;
		...
		case "ring_15":
			dialog.text = "Хех, ты молодец! Ловко раздела"+ GetSexPhrase("лся","лась") +" со всей этой пакостью!";
			link.l1 = "Кто-то говорил мне о 'четырёх-пяти' крабах... Не напомнишь, кто это был?";
			link.l1.go = "ring_16";
		break;

		case "ring_16":
			dialog.text = ""+ GetSexPhrase("Дружище","Подруга") +", я же не лазил в трюм их пересчитывать. Я смотрел снаружи, в пролом, и насчитал именно столько. Так что у меня и в мыслях не было тебе лгать.";
		...
		case "ring_wait":
			dialog.text = "Погоди, "+ GetSexPhrase("дружище","подруга") +", я ещё не всё осмотрел, что хотел. Поройся получше, может, ещё что найдёшь.";
			link.l1 = "Хорошо...";
			link.l1.go = "exit";
...
/* 			for (i=1; i<=3; i++)
		...
		case "ring_28":
			dialog.text = "И снова ничего значительного... Нашёл пару интересных мелочей, и всё. Но это нормально - на палубах ценностей обычно не бывает. Все самоё лучшее лежит в каютах. Вон там - вход в каюту капитана. Пойдём, пороемся там...";
			link.l1 = "Смотри аккуратнее - чёрт его знает, что там может скрываться. Может, лучше я пойду "+ GetSexPhrase("первым","первая") +"?";
			link.l1.go = "ring_29";
		break;

		case "ring_29":
			dialog.text = "Пойдём, "+pchar.name+". Не всё же время мне у тебя за спиной прятаться. И в самом деле - стыдно...";
			link.l1 = "Ну, пошли...";
			link.l1.go = "ring_30";
		break;
		...
		case "ring_31":
			dialog.text = "Кажется, мне повезло - не оказалось ни крабов, ни какой-либо прочей погани. А теперь перевернём всю эту каюту вверх дном - клянусь честью, я уже начинаю злиться: кроме двух десятков дублонов и всякой никчёмной мелочёвки я пока ничего не нашёл!";
		...
		case "ring_33":
			dialog.text = "Ха-ха! Как я и думал! Знаю я замашки этих торгашей - устраивать тайники и прятать там корабельную казну! Капитан этого пинаса был хитрый пройдоха, но меня не проведёшь! Смотри, что мне удалось добыть! Сам"+ GetSexPhrase("","а") +" бы ты в жизни не "+ GetSexPhrase("нашёл","нашла") +" этого схрона!";
			link.l1 = "Ну-ка, покажи!";
			link.l1.go = "ring_34";
		break;

		case "ring_34":
			dialog.text = "Сорок тысяч пиастров, два сундука с дублонами, мешочек изумрудов и куча драгоценностей! Богатенький был владелец... Ладно, это хотя бы окупит наши с тобой труды, хотя я рассчитывал на большее. Делим по-братски, напополам, как договаривались.";
		...
		case "ring_36":
			...
			else
			{
				...
				{
					link.l1 = "Хорошо. Но я "+ GetSexPhrase("нашёл","нашла") +" кое-что ещё - судовой журнал капитана этого корабля.";
					link.l1.go = "ring_38";
				}
				else
				{
		...
		case "ring_38":
			dialog.text = "Ты его "+ GetSexPhrase("прочёл","прочла") +"?";
			link.l1 = "Пока нет...";
			link.l1.go = "ring_39";
		break;

		case "ring_39":
			dialog.text = "Тогда давай читать! Там могут содержаться важные сведения. Да и просто интересно узнать, почему этот корабль оказался тут. Читай!";
		...
		case "ring_41":
			...
			link.l1 = "Вкратце: этот пинас пересекал Карибское море от Южного Мэйна в сторону Новой Испании. Капитан взял себе в жёны принцессу-индианку из племён нижнего Ориноко, а она показала ему, где их люди делали жертвоприношения богам...";
			link.l1.go = "ring_42";
		break;

		case "ring_42":
			dialog.text = "";
			link.l1 = "Жертвоприношения были в виде золотых самородков. Наш отважный капитан, безусловно, разграбил этот золотой запас, и вместе со старпомом и квартирмейстером, перенёс тайком от команды в трюм корабля, сложил в один из ящиков и присыпал копрой...";
			link.l1.go = "ring_43";
		break;

		case "ring_43":
			dialog.text = "";
			link.l1 = "Но, видно, грабёж священного индейского места не прошёл бесследно: корабль попал в мёртвый штиль, начала заканчиваться пресная вода, на корабле вспыхнула эпидемия лихорадки, напали пираты. Матросы обвинили во всем индианку, жену капитана, и потребовали выбросить её за борт. Кэп, конечно, отказался, и команда подняла бунт...";
			link.l1.go = "ring_44";
		break;
		...
		case "ring_50":
			dialog.text = "Вот и всё... Спасибо, "+pchar.name+", что "+ GetSexPhrase("согласился","согласилась") +" пойти со мной на дело. Без тебя бы ничего не вышло. Теперь можно озадачиться постройкой тартаны, чтобы уплыть на архипелаг... а, ладно. Не буду думать об этом сегодня\nУ меня ещё очень много работы - перетаскать своё золото по безопасным местам. Пока, "+ GetSexPhrase("дружище","подруга") +", желаю удачно распорядиться своей долей!";
...
//----------------------------------------- специальные реакции -----------------------------------------------
		...
		case "Man_FackYou":
			dialog.text = LinkRandPhrase("Что ты там копаешься, а? Да ты вор!", "Вот это да! Чуть я загляделся, а ты сразу в сундук с головой!", "По сундукам шарить вздумал"+ GetSexPhrase("","а") +"?! Тебе это даром не пройдёт!");
			link.l1 = "А-ать, дьявол!!!";
			link.l1.go = "fight";
		break;

		case "Woman_FackYou":
			dialog.text = "Ах, вот, значит, как?! По сундукам шарить вздумал"+ GetSexPhrase("","а") +"?! Тебе это даром не пройдёт!";
			link.l1 = "Вот дура!..";
		...
		case "CitizenNotBlade":
			...
			{
				dialog.text = NPCharSexPhrase(NPChar, "Осторожней на поворотах, "+ GetSexPhrase("приятель","девчонка") +", когда бежишь с оружием в руках. Я ведь могу и занервничать...", "Мне не нравится, когда "+ GetSexPhrase("мужчины","девушки") +" ходят передо мной с оружием на изготовку. Это меня пугает...");
