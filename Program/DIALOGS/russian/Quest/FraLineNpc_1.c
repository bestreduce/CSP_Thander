...
//>>>>>>>>>>>>========= Разброс диалогов по персонажам =====================
            ...
            if (pchar.questTemp.State == "Fr1Legran_afterFightBand")// квест №1, Легаран после боя.
            {
    			...
    			link.l1 = "Месье, я доставил"+ GetSexPhrase("","а") +" вас до бухты, моя задача выполнена...";
    			link.l1.go = "Step_F1_3";
    			link.l2 = "А мы уже пришли! Я решил"+ GetSexPhrase("","а") +", что твои денежки мне само"+ GetSexPhrase("му","й") +" пригодятся.";
			...
			if (npchar.id == "DeLeiva")// квест №3, база с мужем донны Анны
            {
				...
				link.l1 = "Вот так встреча! А в чём дело?!";
            ...
            {
				dialog.text = NPCStringReactionRepeat("Сеньор"+ GetSexPhrase("","ита") +", вы уже второй раз спасаете мою жизнь. Знайте, что в моём лице вы обрели верного друга.",
					         "Здравствуйте снова, капитан. Рада вас видеть.", "И ещё раз здравствуйте капитан...", "Который раз на дню: здравствуйте, капитан...", "block", 0, npchar, Dialog.CurrentNode);
				link.l1 = HeroStringReactionRepeat("Сеньора, совершенно случайно близ Гаваны мне встретились два кастильских дворянина, и знали бы вы, как они о вас выражались... По всему видать, донна Анна, что память о вас бережно хранится в Кастилии.",
					      "И я вас рад"+ GetSexPhrase("","а") +" видеть так же.", "И ещё раз я рад"+ GetSexPhrase("","а") +" вас видеть...", "Здравствуйте, сеньора...", npchar, Dialog.CurrentNode);
            ...
            {
				dialog.text = NPCStringReactionRepeat("Судар"+ GetSexPhrase("ь","ыня") +", вас что, поучить хорошим манерам?! Ко мне в дом без приглашения не смеет входить никто!",
					         "Мы вроде бы всё обсудили...", "Ещё что-то?", "Опять то же самое, сколько можно?!", "block", 0, npchar, Dialog.CurrentNode);
				link.l1 = HeroStringReactionRepeat("Моё приглашение при мне. Вот оно - это срочный пакет от генерал-губернатора д'Ожерона.",
            ...
            {
    			dialog.text = "Ты наш"+ GetSexPhrase("ел","ла") +" наши реалы?";
    			link.l1 = "Да, всё в порядке. К счастью, мы успели вовремя - деньги у меня.";
			...
			if (npchar.id == "Bonrepo" && pchar.questTemp.State == "Fr8ThreeCorsairs_toBonrepo") //Квест №8. Маркиз Бонрепо в Бас-Тере
            {
    			...
    			link.l1 = "Капитан " + GetFullName(pchar) + ". Я прибыл"+ GetSexPhrase("","а") +" к вам по распоряжению генерал-губернатора д`Ожерона.";
...
//********************************* Доставка Леграна до Мартиники. Квест №1 **********************************
 		case "Step_F1_1":
			...
			link.l1 = "Меня зовут "+ GetFullName(pchar) +". Я прислан"+ GetSexPhrase("","а") +" к вам генерал-губернатором д'Ожероном. Имею поручение тихо и незаметно доставить вас до Мартиники в бухту Ле Марен.";
			link.l1.go = "Step_F1_2";
		break;
 		case "Step_F1_2":
			dialog.text = "Если вам доверяет генерал-губернатор, то доверяю и я. Когда мы отправляемся?";
			link.l1 = "Как можно скорее. Ждите меня на моём корабле.";
		...
		break;
 		case "Step_F1_4":
			dialog.text = "Эти деньги ваши, извольте получить.\n"+
				          "Прощайте и будьте здоровы, "+ GetSexPhrase("месье","мадемуазель") +".";
		...
		break;
 		case "Step_F1_5":
			dialog.text = "Ах ты, мерзав"+ GetSexPhrase("ец","ка") +"! Я сообщу генерал-губернатору, как"+ GetSexPhrase("ой негодяй втёрся","ая негодяйка втёрлась") +" к нему в доверие!";
...
//********************************* Арест ГГ у Стэвезанта. Квест №2 **********************************
 		case "Step_F2_1":
			dialog.text = "Эй, тебе чего здесь надо, чего встал"+ GetSexPhrase("","а") +"? Мы тебя выпить не приглашали.";
			link.l1 = "Меня зовут капитан " + GetFullName(pchar) + ", я только что с Тортуги... Не желаете ко мне в команду?";
		...
		break;
 		case "Step_F2_7":
			dialog.text = "А кто ты так"+ GetSexPhrase("ой","ая") +", чтобы требовать ответа?";
			link.l1 = "Я уж представлял"+ GetSexPhrase("ся","ась") +", если мне не изменяет память. Или вам неизвестно, что такое Тортуга и кто такие флибустьеры?!";
    		link.l1.go = "Step_F2_8";
		break;
 		case "Step_F2_8":
			dialog.text = "Кто такие флибустьеры - нам известно. Мы называем вас ладронами.";
			link.l1 = "Так нас называют только испанцы...";
    		link.l1.go = "Step_F2_9";
		break;
 		case "Step_F2_9":
			dialog.text = "Верно... Амиго, святой Антоний предоставил нам возможность причастить капитана ладронов - благо "+ GetSexPhrase("он один","она одна") +". Не будем терять времени...";
			link.l1 = "Вот те на, испанцы! А что за ваш галеон, вы не ответили!";
    		link.l1.go = "Step_F2_10";
		break;
 		case "Step_F2_10":
			dialog.text = "Мы ждём наш галеон здесь, он вот-вот должен бросить якорь. Теперь ты знаешь все, но знание это тебе не пригодится...";
...
//********************************* Драка с мужем де Лейва. Квест №3 **********************************
 		case "Step_F3_1":
			dialog.text = "Нет, ну как вам нравится... В чужой дом посреди ночи вламывается "+ GetSexPhrase("мужчина","какая-то посторонняя девка") +", а потом спрашивает: 'А в чём дело?!'\n"+
				          "Не стройте из себя идиот"+ GetSexPhrase("а","ку") +"! Моя жена заперта, я нашёл у неё это чёртово кольцо д'Ожерона.";
		...
		break;
 		case "Step_F3_2":
			dialog.text = "Начнём трапезу!";
			link.l1 = "Начнём, пожалуй...";
...
//********************************* Месть Анне де Лейва. Квест №5 **********************************
 		case "Step_F5_1":
			dialog.text = "Я получила письмо из Гаваны от моей подруги Инес де лас Сьеррас. Генерал-губернатор Гаваны предпринял расследование смерти моего мужа и убеждён, что в этом повинна я. Ах, так оно и есть! Но мои соотечественники замыслили месть, мне грозит гибель или похищение, и я не знаю, что страшнее.\n"+
		...
		break;
 		case "Step_F5_3":
			...
			link.l1 = "Я так понял"+ GetSexPhrase("","а") +", вы замыслили отомстить за дона Хосе?";
    		link.l1.go = "Step_F5_4";
		break;
 		case "Step_F5_4":
			dialog.text = "Немедленно говори, кто ты. Иначе я отрежу тебе твой длинный язык.";
			link.l1 = "Я "+ GetSexPhrase("тот","та") +", кто отправил"+ GetSexPhrase("","а") +" уважаемого дона Хосе на тот свет. Ещё и слуг его туда же, вслед за хозяином.";
		...
		break;
 		case "Step_F5_6":
			dialog.text = "А тебя уже нет... Вперёд, брат, покончим с "+ GetSexPhrase("этим мерзавцем","этой дрянью") +"!";
...
//********************************* Франсуа Олоне. Квест №6 **********************************
		...
		break;
 		case "Step_F6_2":
			dialog.text = "Полагаю, раз вы здесь, значит, испанцы не скоро дождутся своего военного корабля, ха-ха! Да, они спят и видят, как бы доказать связи французского генерал-губернатора с флибустьерами. Так что, доставив письмо в целости, вы оказали и мне, и месье д`Ожерону немалую услугу. Награда будет соответствующей. Кстати, как вы предпочитаете её получить?";
		...
		break;
 		case "Step_F6_4":
            if (GetCompanionQuantity(pchar) == 1)   // Заставляем геймера избавиться от взятых кубиков.
            {
				...
				{
					dialog.text = "Ну что же, ты готов"+ GetSexPhrase("","а") +" к тому, чтобы занять место в нашей эскадре. Добро пожаловать и вперёд к Кумане!";
					link.l1 = "Очень хорошо! Франсуа, каков план действий на эту компанию?";
    				link.l1.go = "Step_F6_5";
				}
				else
				{
					dialog.text = "Ну, и где ты пропадал"+ GetSexPhrase("","а") +" столько времени?";
					link.l1 = "Да вот, избавлял"+ GetSexPhrase("ся","ась") +" от кораблей, как ты и сказал.";
    				link.l1.go = "Step_F6_7";
				}
            }
            else
            {
				dialog.text = "Коллега, но я могу взять в это дело тебя одн"+ GetSexPhrase("ого","у") +", на условиях, что твоя доля - четвёртая часть. Больше мне компаньоны не требуются. Поэтому избавься от лишних кораблей, если хочешь участвовать. И побыстрей!";
				link.l1 = "Хорошо, Франсуа, я скоро вернусь.";
    			link.l1.go = "exit";
				link.l2 = "Не, на такой расклад я не соглас"+ GetSexPhrase("ен","на") +". Поэтому я, пожалуй, лучше возьму таки свои деньги... ";
    			link.l2.go = "Step_F6_WantMoney";
				NextDiag.TempNode = "Step_F6_4";
             }
		break;
 		case "Step_F6_5":
			dialog.text = "План достаточно прост. Мы приходим туда и берём Куману штурмом. Затем находим золото, делим и разбегаемся каждый по своим делам.";
			link.l1 = "Превосходно. Ну что же, я готов"+ GetSexPhrase("","а") +".";
    		link.l1.go = "Step_F6_6";
		break;
 		case "Step_F6_6":
			dialog.text = "Тогда вперёд на Куману, и как можно быстрей!";
		...
		break;
		case "Step_F6_7":
			dialog.text = "Избавил"+ GetSexPhrase("ся","ась") +"?";
			link.l1 = "Да, конечно.";
		...
		break;
 		case "Step_F6_9":
			dialog.text = "Очень просто, "+ GetSexPhrase("мон шер","дорогуша") +". Золота в Кумане уже нет, его вывезли. Так что в нашем случае потерянное время стоило нам 200 тысяч реалов. Вот так-то...";
			link.l1 = "Чёрт возьми, досадно... Ну что же, тогда я заберу те деньги, которые мне причитаются за доставку письма.";
    		link.l1.go = "Step_F6_WantMoney";
		break;
		...
		break;
 		case "Step_F6_11":
			dialog.text = "Я так не думаю, "+ GetSexPhrase("мон шер","дорогуша") +". А думаю я то, что это из-за тебя не успели вовремя!";
			link.l1 = "Спорное утверждение, друг Олоне. Очень спорное...";
    		link.l1.go = "Step_F6_12";
		break;
 		case "Step_F6_12":
			dialog.text = "Полагаю, пришло время разрешить все споры. К оружию, судар"+ GetSexPhrase("ь","ыня") +"! Попробуем твою кровь на вкус...";
			link.l1 = "Видит Бог, я не хочу с тобой драться!";
    		link.l1.go = "Step_F6_13";
		break;
 		case "Step_F6_13":
			dialog.text = "Тогда я убью тебя как трусливую собаку! Последний шанс - берись за оружие.";
		...
		break;
 		case "Step_F6_14":
			if (sti(pchar.money) >= 200000)
			{
				...
				link.l1 = "Прощай, Олоне. Я похозяйничаю ещё немного в городе...";
    			link.l1.go = "Step_F6_15";
				link.l2 = "А я вот думаю, что не обязан"+ GetSexPhrase("","а") +" с тобой делиться. Всю работу сделал"+ GetSexPhrase("","а") +" я!";
    			link.l2.go = "Step_F6_20";
			}
			else
			{
				dialog.text = "Деньги ты наш"+ GetSexPhrase("ел","ла") +", а у тебя их нет в наличии. Хитрить вздумал"+ GetSexPhrase("","а") +"?!";
		...
		break;
 		case "Step_F6_15":
			...
			link.l1 = "Хорошо, Франсуа. Будь здоров...";
		...
		break;
 		case "Step_F6_17":
			dialog.text = "А я так думаю, хитришь, "+ GetSexPhrase("приятель","подруга") +"... Но есть способ это выяснить - доставай свой клинок.";
			link.l1 = "Схватка?";
		...
		break;
 		case "Step_F6_20":
			dialog.text = "Вот, значит, как?! Ну, тогда готовься к худшему...";
			link.l1 = "Всегда готов"+ GetSexPhrase("","а") +", Франсуа...";
...
//********************************* Маркиз Бонрепо. Квест №8 **********************************
 		case "Step_F8_1":
			dialog.text = "Ага, наконец-то! А я уже начал волноваться. Генерал-губернатор д'Ожерон сообщил мне о вас много лестного. Я надеюсь, что вы полностью оправдаете его и моё доверие. И учтите: все, что вы здесь услышите, никогда не должно стать известным кому-либо, кроме вас.";
			link.l1 = "Я предупреждён"+ GetSexPhrase("","а") +" об этом...";
		...
		break;
 		case "Step_F8_6":
			dialog.text = "Тогда мне придётся отправить вас в тюрьму. Вы ведь, "+ GetSexPhrase("милейший","дорогая") +", стали обладател"+ GetSexPhrase("ем","ьницей") +" государственной тайны! С такими знаниями нелояльные люди на свободе не разгуливают.";
		...
		break;
 		case "Step_F8_9":
			...
			link.l1 = "Э-э... а... "+ GetSexPhrase("Баронский титул","Титул баронессы") +"? А как быть с тратами, которые я пон"+ GetSexPhrase("ёс","есла") +"?..";
    		link.l1.go = "Step_F8_10";
		break;
 		case "Step_F8_10":
			dialog.text = "Ох, пират всегда останется пиратом! Ха-ха!\n"+
				          "Поверьте, "+ GetSexPhrase("милейший","дорогая") +", я даю вам гораздо больше, чем презренный металл! Я даю вам честь и уважение самого короля! А сейчас не смею вас задерживать, вас ждёт генерал-губернатор д'Ожерон. Кажется, у него для вас опять какое-то важное дело... Прощайте.";
		...
		break;
 		case "Step_F8_11":
			...
			AddQuestUserData("Fra_Line_8_ThreeCorsairs", "sSex", GetSexPhrase("стал дворянином","стала дворянкой"));
...
//********************************* Конец войне. Квест №12 **********************************
		...
		break;
 		case "Step_F12_5":
			dialog.text = "Барон"+ GetSexPhrase("","есса") +", не делайте из меня идиота, я же не требую с вас отчёта о захваченных в городах трофеях. Это и есть ваш гонорар.";
