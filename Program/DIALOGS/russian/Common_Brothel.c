...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			{
				dialog.text = "Ты взял"+ GetSexPhrase("","а") +" деньги, но пожертвования не сделал"+ GetSexPhrase("","а") +"! Мерзав"+ GetSexPhrase("ец","ка") +", одним словом. Я не буду тебя обслуживать в моём заведении. Пош"+ GetSexPhrase("ел","ла") +" вон!";
			...
			if (npchar.questChurch == "taken")
			{
				...
				{
					dialog.text = "Ты ещё не отдал"+ GetSexPhrase("","а") +" деньги в церковь. Займись этим, времени у тебя - текущий день! Ты обещал"+ GetSexPhrase("","а") +"!";
			...
			if (npchar.questChurch == "yes")
			{
				...
				link.l1 = "Да, я всё сделал"+ GetSexPhrase("","а") +". Падре просил передать тебе, что твои деньги приняты церковью.";
			...
			if (rand(3) == 1 && pchar.questTemp.different == "free" && PChar.sex != "woman" && GetNpcQuestPastDayWOInit(npchar, "questSex") > 180 && !CheckAttribute(npchar, "quest.selected") && !CheckAttribute(npchar, "quest.NotGoneToSex.over"))
			{
				...
				{
					dialog.text = "В прошлый раз, когда я ждала тебя в комнате наверху, ты не пришёл... Ты упустил свой шанс.";
					link.l1 = "Эх, жаль...";
					link.l1.go = "exit";
			...
			if (npchar.quest.meeting == "0")
			{
				dialog.text = RandPhraseSimple(TimeGreeting() + ". Добро пожаловать в дом терпимости. Меня зовут " + npchar.name + ", я здесь заведую всем. "+ GetSexPhrase("Чем могу быть полезна?, " + GetAddress_Form(NPChar) + "","Признаюсь, немного удивлена видеть вас здесь, " + GetAddress_Form(NPChar) + ", но смею заверить: мы оказываем услуги не только мужчинам.") +"",
				                               TimeGreeting() + ". Приветствую вас, "+ GetSexPhrase("незнакомец","девушка") +", в моём скромном заведении. Позвольте представиться, я "+ NPChar.Name + " - владелица этого приюта обиженных лаской мужчин. "+ GetSexPhrase("Чем я могу быть вам полезна?","Хотя для женщин у нас тоже кое-что есть...") +"");
        ...
        case "Hostess_Choice_2":
			dialog.text = "Тогда тебе нужно назвать её имя ещё раз, возможно я пойму, о ком идёт речь.";
			Link.l1.edit = 9;
			Link.l1 = "";
			Link.l1.go = "Hostess_Choice_1";
			Link.l2 = "Пожалуй, мне лучше уточнить её имя. Я позже переговорю с тобой на эту тему.";
		...
		break;
		case "toChurch_1":
			dialog.text = "Хорошо... Но это не значит, что о душе своей я не думаю. В общем, я хочу сделать пожертвование церкви. Но появится там мне сложно - косые взгляды, перешёптывания женушек...";
		...
		case "ResultChurch_1":
			dialog.text = "Слава Господу, для меня ещё не все потеряно в этой жизни!.. Спасибо тебе.";
		...
		case "SpeakHistory":
			...
			{
				link.l1 = "Как-то брали мы одного торговца, славная добыча, доложу я тебе, у него оказалась в трюме! Золото и серебро!!! Клянусь громом, так и было! Ха-ха! А ещё у него на борту были славные курочки, такие ладные, фигуристые, мы их хорошенько пощипали...";
				link.l1.go = "SpeakHistory_2";
			}
		break;
		case "SpeakHistory_1":
			dialog.text = "О-о! На тебя нападала акула! Как же тебе удалось спастись?!";
			link.l1 = "Ну, тогда по порядку. Попали мы как-то в полный штиль, ни ветерка, и жара страшенная стоит. Решили мы искупнуться, чтоб освежиться немного. Это такое наслаждение, доложу я тебе, окунуться после изнуряющей жары в воду. И так я увлёкся плаванием и нырянием, что не услышал криков вахтенного: '" + GetAddress_Form(PChar) + ", берегитесь! Акула! Акула!!!'. И тут я её увидел! Я как раз нырнул и заметил её силуэт под собой. Ну и громадина!!! В пять раз больше меня! В этот момент она разворачивается и идёт прямо на меня!..";
			link.l1.go = "SpeakHistory_3";
		break;
		case "SpeakHistory_2":
			dialog.text = "Хватит! С меня довольно! Ты наглый, самовлюблённый тип! Убирайся с глаз моих!";
			link.l1 = "Хм, за что?..";
			link.l1.go = "exit";
		break;
		case "SpeakHistory_3":
			dialog.text = "О, боже! И что же?!";
			link.l1 = "Я понимаю, что моё спасение в моих руках, разворачиваюсь и плыву, что есть мочи к шлюпке, там ребята пытаются выстрелами отогнать акулу, но такого монстра разве отгонишь... До шлюпки оставалось всего семь ярдов, а за моей спиной уже показался огромный плавник...";
		...
		break;
		case "SpeakHistory_5":
			dialog.text = "И ты её зарубил! Правда?! Ведь ты зарубил этого монстра?";
		...
		case "SpeakHistory_10":
			dialog.text = "Ах, как вы благородны! Я даже на мгновение забыла, хозяйкой какого заведения являюсь. Наверное, вы действительно немного грубоваты и... прямолинейны, но в вас есть нечто влекущее...  И потом, мне всё-таки любопытно посмотреть на ваши шрамы, пойдёмте наверх, в комнату, там нас никто не побеспокоит... И там, возможно, вы тоже захотите что-нибудь посмотреть...";
		...
		case "SpeakHistoryTwo_1":
			...
			link.l1 = "Я люблю риск! Натиск и напор, вот мой девиз! Как только я тебя увидел, сразу понял, ты должны стать моей! Такой очаровательной фигурки я ещё не встречал! Какие формы, боже мой!..";
		...
		case "SpeakHistoryTwo_5":
			...
			link.l2 = "Мне понадобилась уйма времени, чтоб понять и увидеть эту истину. Мир без тебя скуден и беден, а при твоём появлении все оживает, звонче поют птицы, ласково шелестит прибой и в лучах утреннего солнца, блестят росой на лепестках распускающиеся цветы...";
		...
		case "SpeakHistoryTwo_10":
			dialog.text = "Да? И о чём же ещё? Даже боюсь предположить.";
			link.l1 = "О твоей прелестной шейке, и о твоём немыслимом декольте, и у меня перехватывает дыхание, как только я подумаю, сколь прекрасен твой бюст!";
			link.l1.go = "SpeakHistoryTwo_11";
		break;
		case "SpeakHistoryTwo_11":
			dialog.text = "Наглец!..";
			link.l1 = "Быть может... И ты, наверное, прогонишь меня прочь, но... глядя на твоё прекрасное платье, я непрестанно думаю о том, что под ним. Ой, прости, я забылся...";
		...
		case "SpeakHistoryTwo_13":
			...
			link.l1 = "Так я прощён?";
			link.l1.go = "SpeakHistoryTwo_14";
		break;
		case "SpeakHistoryTwo_14":
			dialog.text = "Быть может... Здесь слишком душно, давай пройдём наверх, там прохладней, там и продолжим... разговор.";
        ...
        case "Horse_talk":
			...
			link.l1 = HeroStringReactionRepeat("Хорошо, "+ GetSexPhrase("красавица, ","") +"я понял"+ GetSexPhrase("","а") +".", "А-а, да, конечно...",
                      ""+ GetSexPhrase("Можешь не сомневаться, красавица, я упрям и силён, как бык!","Да-да...") +"", ""+ GetSexPhrase("Вот дьявол, что-то упустил... Прости, милая.","Хорошо, хорошо.") +"", npchar, Dialog.CurrentNode);
			link.l1.go = "exit";
        ...
        case "Horse_2":
			...
			{
				dialog.text = LinkRandPhrase(""+ GetSexPhrase("Знаешь что я тебе скажу, милый мой? Не нужно мне пустых разговоров. Займись делом!","Ну вот только этого мне не хватало - комплиментов от женщин!") +"", ""+ GetSexPhrase("Ты тоже воображаешь, что все женщины любят ушами? Хм, это очень зря... Если хочешь меня - плати хозяйке заведения, а трепотня мне не нужна.","Девушка, не растрачивайтесь на пустые слова. Хотите дел - платите...") +"", ""+ GetSexPhrase("Ещё один любитель чувственности... Плати - и я буду твоей, но только без этого нежного бреда!","Ты чего, дорогуша? Хочешь удовольствия - плати, а не занимайся болтовнёй!") +"");
		...
		break;
        case "Horse_3":
			dialog.text = characters[GetCharacterIndex(npchar.city + "_Hostess")].name + " все оформляет в своём кабинете. Иди к ней"+ GetSexPhrase(", мой герой,","") +" и назови моё имя - " + npchar.name + ". Я буду тебя ждать...";
		...
		break;
        case "Horse_4":
			dialog.text = NPCStringReactionRepeat("Ты уже оплатил"+ GetSexPhrase("","а") +" девушку, она ждёт тебя, поднимайся наверх.",
		...
		case "Priest1":
			...
			if(CheckAttribute(PChar, "chr_ai.HeavyTrauma"))//лечение травмы в борделе - Gregg
			{
				...
				Log_Info("Вы оправились от тяжёлой травмы");
        ...
        case "Horse_ReadyFack":
			...
			switch(npchar.quest.choice)
			{
				...
				break;
				case "2":
					dialog.text = "А-а-а, приш"+ GetSexPhrase("ёл","ла") +", наконец-то. Ну, не будем терять времени!";
        ...
        case "Horse_AfterSex_2":
			...
			{
				dialog.text = LinkRandPhrase("Заходи к нам ещё как-нибудь...", "До свидания и до новых встреч...", "Будем рады видеть тебя у нас ещё...");
				Link.l1 = "Ладно...";
				Link.l1.go = "exit";
				NextDiag.TempNode = "Horse_AfterSex_2";
			}
		...
		case "TakeMayorsRing_S4":
			dialog.text = "Ну а я то здесь при чём? Если он бы не был не в себе, то это его проблемы!";
...
//********************************* Диалоги девки из борделя ФФ **********************************
		...
		break;
  		case "Step_Hol8_3":
			dialog.text = ""+ GetSexPhrase("Хорошо, отвечу на любой твой вопрос. Но только наверху - здесь не место для разговоров.","Хи-хи-хи... Тебе нужна информация - мне нужны деньги. Хочешь, могу дать не только информацию, но и кое-что ещё, если поднимемся наверх... Я тебя знаю. Только это будет дороже, хи-хи...") +"";
		...
		break;
		case "Step_Hol8_6":
			dialog.text = "Повертели они её в руках. На обложке распятие выбито, а так невозможно и понять, что за книга. Написана она на незнакомом языке, никто не смог даже примерно сказать, что за письмена.";
			link.l1 = "Хм, раз она продаётся, то я хотел"+ GetSexPhrase("","а") +" бы её приобрести. Как я могу найти этого корсара?";
			link.l1.go = "Step_Hol8_7";
    	break;
		case "Step_Hol8_7":
			dialog.text = "Иди прямиком на Тортугу, вот и найдёшь. Как я уже сказала, зовут его Лоран де Граф, он известный флибустьер, голландец.";
			link.l1 = "Спасибо, отправлюсь туда.";
			link.l1.go = "Step_Hol8_8";
    	break;
		case "Step_Hol8_8":
			dialog.text = "Слушай, а что, действительно эта книга стоит денег?";
			link.l1 = "Сложно сказать, смотреть надо. А что де Граф говорил? Где нашёл, что знает о книге?";
