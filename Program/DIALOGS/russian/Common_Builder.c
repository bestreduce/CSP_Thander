void ProcessDialogEvent()
{
	...
	{
		case "First time":
			dialog.text = "Чем могу быть полезен, "+GetSexPhrase("герр капитан","фройлен")+" " + sName + "?";
		...
		case "Hi1":
			dialog.text = "Гутен так, "+GetSexPhrase("герр капитан","фройлен")+"! Вы не поверите, что со мной случилось!";
		...
		case "ColonyBuilding_6":
			dialog.text = "А-а, это вы, "+ GetSexPhrase("герр капитан","фройлен") +"! Строительство идёт полным ходом, йа!";
			link.l1 = "До встречи.";
			link.l1.go = "exit";
		break;

		case "ColonyBuilding_7":
			dialog.text = "Гутен таг, "+ GetSexPhrase("герр капитан","фройлен") +"! С удовольствием вам сообщаю, что строительство вашей колонии завершено! Если вам снова понадобятся мои услуги, вы сможете найти меня в Вашем поселении.";
		...
		case "ColonyBuilding_8":
			dialog.text = "Гутен таг, " + sName + ". Вы что-то хотели?";
			link.l1 = "Нет, ничего. До встречи.";
			link.l1.go = "exit";
		break;
		...
		case "ColonyModification_1":
			dialog.text = "Вот и вы, "+ GetSexPhrase("герр капитан","фройлен") +". Что же, приступим. Перед тем, как мы начнём наше строительство, мы должны определиться с архитектурой будущего города и его зданий.";
		...
		case "ColonyModification_3":
			...
			if(sti(PChar.BuildingColony.Blocks) == 0 && sti(PChar.BuildingColony.Planks) == 0 && sti(PChar.BuildingColony.Mahogany) == 0 && sti(PChar.BuildingColony.Ebony) == 0 && sti(PChar.BuildingColony.Slaves) == 0 && sti(PChar.BuildingColony.Food) == 0)
			{
				dialog.text = "С радостью сообщаю вам, "+ GetSexPhrase("герр капитан","фройлен") +", что все необходимые товары доставлены и что мы начинаем строительство. По моим подсчётам строительство будет идти чуть более, чем полтора месяца, йа!";
		...
		case "ColonyModification_4":
			dialog.text = "С радостью сообщаю, что расширение колонии ''" + PChar.ColonyBuilding.ColonyName + "'' завершено! С вами было приятно работать, "+ GetSexPhrase("герр капитан","фройлен") +". Пожалуй, я побуду тут ещё какое-то время. Если вы захотите снова меня найти - я буду в порту.";
		...
		case "ColonyModification_5":
			dialog.text = "Это вы, "+ GetSexPhrase("герр капитан","фройлен") +" " + sName + "? Ещё раз обсудим детали строительства?";
		...
		case "ColonyFortBuilding_1":
			dialog.text = "Отлично, "+ GetSexPhrase("герр капитан","фройлен") +"! Встретимся в месте под названием ''Пустынный пляж'' - там то и будет заложен фундамент нашего охранного строения.";
			link.l1 = "До встречи.";
		...
		case "ColonyFortBuilding_2":
			dialog.text = "Отлично, "+ GetSexPhrase("герр капитан","фройлен") +"! Встретимся в месте под названием ''Пустынный пляж''.";
			link.l1 = "До встречи.";
			link.l1.go = "ColonyFortBuilding_to_port";
		break;
		...
		case "ColonyFortBuilding_4":
			dialog.text = "Что вы хотели, "+ GetSexPhrase("герр капитан","фройлен") +"?";
			if(sti(PChar.BuildingColony.Blocks) == 0 && sti(PChar.BuildingColony.Planks) == 0 && sti(PChar.BuildingColony.Mahogany) == 0 && sti(PChar.BuildingColony.Ebony) == 0 && sti(PChar.BuildingColony.Slaves) == 0 && sti(PChar.BuildingColony.Food) == 0)
			{
		...
		case "ColonyFortBuilding_5":
			dialog.text = "А-а, это вы, "+ GetSexPhrase("герр капитан","фройлен") +"! Строительство идёт полным ходом!";
			link.l1 = "До встречи.";
			link.l1.go = "exit";
		break;

		case "ColonyFortBuilding_6":
			dialog.text = "Создание охранной базы для вашей колонии завершено! С вами было приятно работать, "+ GetSexPhrase("герр капитан","фройлен") +". Я возвращаюсь домой.";
