void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Node_31":
			dialog.text = "А что мне оставалось делать?! Отец меня замуж выдать хочет за сына ростовщика - этого слюнтяя... потому что у его папаши в сундуках полно пиастров. А я люблю другого! Отведите меня к нему, прошу вас.";
		...
		case "Node_132_4":
			...
			link.l1 = "Не думаю, что она может стоить дороже " + sti(pchar.GenQuest.EncGirl.mapPrice) + " пиастров.";
		...
		case "Node_133":
			...
			{
				link.l2 = "Вот, держи 15000 пиастров, за эти деньги можно, если не целое ожерелье купить, то жемчужины к нему подобрать - уж наверняка.";
				link.l2.go = "Node_135";
			}
		break;
...
//			link.l2.go = "Node_248"
		...
		case "Node_237":
			...
			{
				link.l1 = "Понятно. Вот, держи 5000 пиастров. Этого должно хватить.";
				link.l1.go = "Node_237_1";
			}
			if(makeint(pchar.money)>=25000)
			{
				link.l2 = "Понятно. Вот, держи 25000 пиастров. И отправляйся домой.";
				link.l2.go = "Node_237_2";
			}
			if(makeint(pchar.money)>=35000)
			{
				link.l3 = "Понятно. Вот, держи 35000 пиастров. Снимешь себе каюту 'люкс'.";
		...
		case "Node_250":
			dialog.text = "Не губите, капитан! Отец меня замуж выдать хочет за сына ростовщика - урода слюнявого... потому что у его папаши в сундуках полно пиастров! Лучше я в джунглях останусь, на съедение диким зверям! Не отправляйте в пожизненную кабалу, умоляю!";
