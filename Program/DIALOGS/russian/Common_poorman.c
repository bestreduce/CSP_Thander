...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			if (npchar.quest.meeting == "0")
			{
				...
				if (pchar.questTemp.LSC == "PoorMurderBegin")
				{
					...
					link.l1 = LinkRandPhrase("Всё дело в деньгах...", "Мне пообещали за эту работу оч-ч-чень большое вознаграждение!", "Такова жизнь... А вообще, все дело в корысти, друг мой.");
					link.l1.go = "exit";
					break;
				}
				//<-- если ГГ начал мочить бродяг
				...
				{
					dialog.text = "М-да, горожане разошлись не на шутку. А мне дела нет до этого.";
					link.l1 = "Понятно...";
					link.l1.go = "exit";
					break;
				}
			...
			else
			{
				...
				if (pchar.questTemp.LSC == "PoorMurderBegin")
				{
					dialog.text = LinkRandPhrase("Я слышал, что ты "+ GetSexPhrase("убивал","убивала") +" нищих в других городах. Зачем ты это делаешь?",
						"Мне кажется, я знаю, кто ты. Зачем ты убиваешь нас?",
						"Что тебе нужно от нас? Что мы тебе сделали? Зачем убивать безобидных нищих?");
					link.l1 = LinkRandPhrase("Всё дело в деньгах...", "Мне пообещали за эту работу оч-ч-чень большое вознаграждение!", "Такова жизнь... А вообще, все дело в корысти, друг мой.");
					link.l1.go = "exit";
				...
				if (pchar.questTemp.LSC == "SignOnPoorMurder" || pchar.questTemp.LSC == "PoorIsGood")
				{
					...
					{
						link.l1 = "Я "+ GetSexPhrase("слышал","слышала") +", что ты хотел поговорить со мной.";
		...
		break;
		case "GoodTalk_1_2":
			dialog.text = "Смерти?! Не-е-ет, это вряд ли. Послушай, что за странные речи ты ведёшь? Меня это пугает.";
		...
		case "GoodTalk_1_4":
			...
			link.l1 = "Понимаю. Но я тебя "+ GetSexPhrase("предупредил","предупредила") +".";
			link.l1.go = "exit";
		break;
		...
		case "GoodTalk_2_1":
			...
			link.l1 = "Всех нищих на островах в Карибском море.";
			link.l1.go = "GoodTalk_2_2";
		break;
		case "GoodTalk_2_2":
			dialog.text = "Даже не знаю, что и сказать... И что, очень серьёзно напакостили?";
			link.l1 = "Ага. Настолько, что желают вашей смерти.";
			link.l1.go = "GoodTalk_2_3";
		break;
		case "GoodTalk_2_3":
			dialog.text = "Хо-о-о, вот это да!.. Ну, ты "+ GetSexPhrase("сам","сама") +", наверное, понимаешь, что вреда от нас для окружающих - вонь да блохи. Если из-за этого кто-то готов нас убивать, то я вообще ничего не понимаю в этой жизни!";
			link.l1 = "Вот я и говорю, что мне непонятны истоки такой ненависти.";
			link.l1.go = "GoodTalk_2_4";
		break;
		case "GoodTalk_2_4":
			dialog.text = "Извини, но мне они тоже совершенно неясны. Я в полной растерянности...";
			link.l1 = "Понятно. Ну что же, извини, что "+ GetSexPhrase("отвлёк","отвлекла") +" от дел.";
		...
		case "GoodTalk_3":
			...
			link.l1 = "Я тут "+ GetSexPhrase("столкнулся","столкнулась") +" с довольно неожиданным обстоятельством... В общем, дело в том, что на Карибах есть люди, очень недовольные вашим братом.";
		...
		case "GoodTalk_4":
			...
			link.l1 = "Как ты думаешь, кто может быть заинтересован в исчезновении нищих?";
		...
		break;
		case "PoorHeadMan_1":
			dialog.text = "Ну что же, это хорошо. Насколько я смог понять, ты пытаешься узнать о людях, которым мы здорово досадили. Причём настолько сильно, что они готовы нас убивать. Всё правильно?";
		...
		case "PoorHeadMan_6":
			dialog.text = "А то, что почти все наши - бывшие рабы. Ты думаешь, мы бездельники и лентяи? Ничего подобного, просто по стечению обстоятельств нам довелось пройти через этот ад и потерять здоровье. Белым здесь часто дают свободу, так как нужны колонисты\n"+
				"Но далеко не каждый бывший раб может пахать землю и корчевать пни после месяцев, проведённых в трюме галеона по дороге сюда. А Стэвезант, судя по всему, очень не любит бывших рабов, которые так и не занялись ничем полезным.";
		...
		break;
		case "PoorHeadMan_8":
			dialog.text = "Бумаги, "+ GetSexPhrase("друг мой","миледи") +", деловую переписку. Всё, что я тебе тут наговорил, действительно надумано. Мне кажется, что за предложением этого Траста стоит что-то гораздо более весомое, нежели обычная неприязнь кого бы то ни было.";
			link.l1 = "Хм, я тоже так думаю. Что-то здесь серьёзное, учитывая размах замысла и сумму гонорара.";
		...
		case "FoundLetter":
			...
			link.l1 = "Вот письмо, которое я раздобыл"+ GetSexPhrase("","а") +" в резиденции Стэвезанта. Это переписка между ним и Голландской Вест-Индской компанией. Из него явно следует, что затеяла уничтожение нищих именно данная торговая компания.";
		...
		case "FoundLetter_3":
			...
			link.l1 = "Расскажи подробней об этом Острове. Всё, что знаешь и о чём говорят у вас.";
...
*/
            ...
            link.l1 = "Знаешь, ты оказался прав в предположении, которое "+ GetSexPhrase("высказал","высказала") +" в самом начале нашего общения. Всё дело действительно в Тизере Дэне и его Острове.";
            link.l1.go = "GoLSC_1";
		break;
		case "GoLSC_1":
			dialog.text = "Поясни, пожалуйста!";
			link.l1 = "Я пока не могу много говорить. Но скажу, что Остров Тизера, о котором он так много рассказывал - не выдумка. Это правда, и я намерен"+ GetSexPhrase("","а") +" отправиться туда!";
			link.l1.go = "GoLSC_2";
		break;
		case "GoLSC_2":
