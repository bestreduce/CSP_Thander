void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Node_2":
			dialog.text = RandPhraseSimple("Вы не видели ничего подозрительного в округе?", "Встречался ли вам кто-нибудь, внушающий подозрения, " + GetAddress_Form(NPChar) + "?");
			...
			link.l2 = "Да, офицер, у меня к вам очень важное дело... А ну, защищайтесь, солдафоны!";
			link.l2.go = "exit_fight";
			/* --------------------------------------------------------------------------------- */
		break;
		...
		case "Reason_To_Fast_14":
			dialog.text = "Отлично, тогда с тебя " + sti(pchar.questTemp.ReasonToFast.p5) + " золотых, как и уговаривались.";
