...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "FightInArena_2":
			...
			{
				case 0:
					dialog.text = LinkRandPhrase("Вы проиграли, капитан " + GetFullName(PChar) + "! Возможно, в следующий раз вы окажетесь сильнее!", "Очень жаль, что вы потерпели неудачу, капитан " + PChar.lastname + ".", "Эх, вам совсем чуть-чуть не хватило до победы! Ну что ж, возможно в следующий раз вам повезёт больше.");
		...
		case "FightInArena_5":
			dialog.text = LinkRandPhrase("А вот и вы, капитан " + GetFullName(pchar) + "! Что ж, вы как раз вовремя - все участники в сборе и нет никаких препятствий тому, чтобы начать уже наш турнир.", "Приветствую вас, капитан " + GetFullName(PChar) + ". Вас ждут очень тяжёлые схватки - это я вам обещаю. Ну, если вы, конечно, первый раунд пройдёте. Ха-ха!", "Надеюсь, что вы дадите жару этим зазнайкам, капитан " + PChar.lastname + ".");
			...
			switch(sti(PChar.Arena.Tournament.Win))
			{
				...
				case 1:
					...
					if (sti(pchar.questTemp.tournamentcount) == 3 && nopirate)
					{
						...
						dialog.text = LinkRandPhrase("Да-а, турнир удался на славу. И вы стали его победителем! Признаюсь честно, я не мог оторвать глаз от того, как вы управляетесь с оружием! Теперь эти " + sti(PChar.Arena.Tournament.Money) + " золотых по праву принадлежат вам.", "Капитан, вы - чемпион! Такого умения обращаться с оружием не ожидали даже самые яростные соперники! Возьмите ваш приз - эти " + sti(PChar.Arena.Tournament.Money) + " золотых.", "Да вы их всех чуть в лоскуты не разорвали! Вот это хватка! Эти " + sti(PChar.Arena.Tournament.Money) + " золотых по праву ваши.");
