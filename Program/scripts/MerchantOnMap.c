...
void CaptureMerchant(string temp)
{
    ...
    if (trap)
    {
        ...
        string stext1 = "Прошёл слух, что лих"+ GetSexPhrase("ой пират","ая морская разбойница") +" - "+GetFullName(pchar)+", угодил"+ GetSexPhrase("","а") +" в западню, в которую е"+ GetSexPhrase("го","е") +" заманили "+NationNameSK(sti(MerPrm.ination))+"ие власти. Они подослали к не"+ GetSexPhrase("му","й") +" ''торговое'' судно, в трюме которого спрятались вооруженные до зубов "+NationNameSK(sti(MerPrm.ination))+"ие  солдаты. Но "+pchar.name+" не растерял"+ GetSexPhrase("ся","ась") +", он"+ GetSexPhrase("","а") +" их попросту перебил"+ GetSexPhrase("","а") +" - всех до единого! Вот как"+ GetSexPhrase("ой","ая") +" удал"+ GetSexPhrase("ой пират наш","ая деваха наша") +" "+GetFullName(pchar)+".";
