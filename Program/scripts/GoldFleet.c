...
void GoldFleet()
{
	...
	chref.ship.Crew.Morale = 90 + rand(10);
    Fantom_SetCannons(chref, "war"); //fix
    Fantom_SetBalls(chref, "pirate");//все расходники тут
	AddCharacterGoods(chref, GOOD_POWDER, 10000);
	for(i = 0; i<5; i++) {AddCharacterGoods(chref, i, 6000);}//Qwerry,St. - выдача всех расходников есть в Fantom_SetBalls. Тут можно только снаряды докинуть. 
	AddCharacterGoods(chref, GOOD_SAILCLOTH, 600);
	AddCharacterGoods(chref, GOOD_PLANKS, 2500);
    ...
    int nfreg = (6+rand(1));
    int ngal = 5;//Qwerry,St. - убираю рандомный шестой галеон
    ...
    for(int k = 1; k <=(nfreg + ngal) ; k++)
	{
        ...
        if (k<3 || k==7 || k==8 || k>=(5+ngal))//3 СанФелипе(флагман + 2) впереди 	+ 4-5 усиленных фрегатов замыкают		//Qwerry,St. - ставлю галеоны в середину построения
        {
            ...
            Fantom_SetBalls(sld, "pirate");//все расходники тут
			AddCharacterGoods(sld, GOOD_POWDER, 10000);
			for(i = 0; i<5; i++) { if(k < 3) AddCharacterGoods(sld, i, 6000); else AddCharacterGoods(sld, i, 3500);}
			AddCharacterGoods(sld, GOOD_SAILCLOTH, 600);
			AddCharacterGoods(sld, GOOD_PLANKS, 2500);
        ...
        else
        {
			...
			shref.ship.upgrades.sails = 22;//паруса
			Fantom_SetGoods(sld, "war");//пять случайных товаров в трюме от шоколада до рабов
		...
		sld.ship.Crew.Morale = 80 + rand(20);
