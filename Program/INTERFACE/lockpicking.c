...
void InitInterface_gm(string iniName)
{
	...
	sText2 = "Передвигая мышь вверх-вниз, подбирайте положение отмычки. Когда её угол выбран, нажмите и удерживайте клавишу 'Ctrl' для того, чтобы начать вращать цилиндр. Для открытия замка необходимо, чтобы цилиндр был повернут на 90 градусов."
...
void OpenChest()
{
	...
	// LEO: Вот реально, я рот эбал эту рутину. Чот в 2013 году, когда я начинал моделить кораблики в иксах и делал это каждый день и всирал кучу времени, мне тогда было как то похуй. Сейчас чот мне как то немного жалко потраченного времени, конкретно на такую вот рутину. Но сук, это же и для себя делается в первую очередь. Один раз сделать ЗБС и потом играть-кайфовать. Т.ч. я всё ещё в деле и едем дальше :)
...
void IDoExit(int exitCode, bool bClear)
{
	...
	if((rand(99)+1) <= 10*(MOD_SKILL_ENEMY_RATE*3/2) && opened == false) // доработка взлома. Шанс поломки отмычки при отмене взлома
    {
	    TakeNItems(pchar, "Lockpick", -1);
