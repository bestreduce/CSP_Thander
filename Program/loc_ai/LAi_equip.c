...
string LAi_NPC_EquipBladeSelection(int rank, bool notuseinput)
{
	...
	if (rank >= (15 - MOD_SKILL_ENEMY_RATE*3)) // ранг ГГ 15+ (синее сплошняком) - минусуются ранги, в зависимости от сложности. Чем выше сложность, тем раньше начнут появляться бесы с ёбо-пушками.
	{
		min = 18;
		max = 31;
	}
	if (rank >= (25 - MOD_SKILL_ENEMY_RATE*3)) // ранг ГГ 25+ (синее+красное, примерно поровну) LEO: Не, я имел ввиду именно с 25 ранга ГГ, а не с 30. Там имелось ввиду, что к 30 рангу уже лейт гейм, а ставить жёска надо именно с 25 :)
	{
		min = 18;
		max = 39;
	}
...
string LAi_Soldier_EquipBlade(int rank)
{
	...
	if (prank >= (15 - MOD_SKILL_ENEMY_RATE*3)) // ранг ГГ 15+ (синее сплошняком)
	{
		min = 18;
		max = 31;
	}
	if (prank >= (25 - MOD_SKILL_ENEMY_RATE*3)) // ранг ГГ 25+ (синее+красное, примерно поровну) LEO: Не, я имел ввиду именно с 25 ранга ГГ, а не с 30. Там имелось ввиду, что к 30 рангу уже лейт гейм, а ставить жёска надо именно с 25 :)
	{
		min = 18;
		max = 39;
	}
