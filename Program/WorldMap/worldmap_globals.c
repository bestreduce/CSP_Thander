...
#define WDM_MAP_ENCOUNTERS_TO_SEA_SCALE		40	// boal	18.02.06 80 - это много, нужно обязательнй бой, когда догнали	// 80 VANO был тут, чисто для того чтобы корабли ближе появлялись в море, а то они на горизонте тусуются
...
bool wdmCreateFollowShipByIndex(float kSpeed, int index, ref encID, int timeOutInDays)
{
	...
	bool res = false;
	bool sdps = false;
	if (CheckAttribute(pchar,"SeaDogProfessionalSwitch") && pchar.SeaDogProfessionalSwitch == 1) sdps = true;
	bool klas = (sdps) && (GetCharacterShipClass(pchar) < 5);
	//log_info(""+klas);
	if (klas || (GetNationRelation2MainCharacter(iNation) != RELATION_ENEMY) || (rand(100) < (GetCharacterSPECIAL(pchar, "LUCK") * 3)))
	{
